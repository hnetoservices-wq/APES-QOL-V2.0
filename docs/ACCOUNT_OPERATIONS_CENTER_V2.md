...
The village dashboard bridge already exposes account village data including:
...
The replacement must preserve the public API exposed through `window.APES_ACCOUNT_OPERATIONS_CENTER` and the legacy `window.APES_VILLAGE_PALETTE` alias while integrations are migrated.
...
The old Account Operations Center lived inside the Village Palette implementation. The replacement separates responsibilities into dedicated data, rendering, actions and controller modules.

### Stage 1 — Structural split + shell
- Dedicated Account Operations Center modules
