(() => {
  ...
  const MOVEMENTS = Object.freeze([{
    group: 'Roman troops',
    key: 'roman-legionnaire',
    label: 'Legionnaire',
    speed: 6,
    type: 'troop'
  }, {
    group: 'Roman troops',
    key: 'roman-praetorian',
    label: 'Praetorian',
    speed: 5,
    type: 'troop'
  }, {
    ...
    group: 'Roman troops',
    key: 'roman-equites-legati',
    label: 'Equites Legati',
    speed: 16,
    type: 'troop'
  }, {
    group: 'Roman troops',
    key: 'roman-equites-imperatoris',
    label: 'Equites Imperatoris',
    speed: 14,
    type: 'troop'
  }, {
    group: 'Roman troops',
    key: 'roman-equites-caesaris',
    label: 'Equites Caesaris',
    speed: 10,
    type: 'troop'
  }, {
    group: 'Roman troops',
    key: 'roman-battering-ram',
    label: 'Battering Ram',
    speed: 4,
    type: 'troop'
  }, {
    group: 'Roman troops',
    key: 'roman-fire-catapult',
    label: 'Fire Catapult',
    speed: 3,
    type: 'troop'
  }, {
    ...
    group: 'Teuton troops',
    key: 'teuton-clubswinger',
    label: 'Clubswinger',
    speed: 7,
    type: 'troop'
  }, {
    group: 'Teuton troops',
    key: 'teuton-spearfighter',
    label: 'Spearfighter',
    speed: 7,
    type: 'troop'
  }, {
    group: 'Teuton troops',
    key: 'teuton-axefighter',
    label: 'Axefighter',
    speed: 6,
    type: 'troop'
  }, {
    ...
    group: 'Merchants',
    key: 'merchant-roman',
    label: 'Roman Merchant',
    speed: 16,
    type: 'merchant'
  }, {
    group: 'Merchants',
    key: 'merchant-teuton',
    label: 'Teuton Merchant',
    speed: 12,
    type: 'merchant'
  }, {
    group: 'Merchants',
  ...
  function getServerClockSeconds() {
    const clock = document.querySelector('span[i18ndt][full="true"]') || document.querySelector('#servertime[i18ndt], #servertime');
  ...
  function calculateSendPlan(durationSeconds) {
    ...
    if (desiredSeconds === null || !Number.isFinite(durationSeconds)) return {
      valid: false
    };
  ...
  function calculate() {
    ...
    const longDistanceMultiplier = usesTroopModifiers ? (1 + tournamentLevel * 0.1) * (1 + bootsBonus / 100) : 1;
    ...
    if (distance > 20 && longDistanceMultiplier > 1) {
      durationSeconds = 20 / firstSpeed * 3600 + (distance - 20) / longSpeed * 3600;
    }
  ...
  function injectStyles() {
    ...
    style.textContent = `
            ...
            #${BUTTON_ID} svg{width:17px!important;height:17px!important;fill:none!important;stroke:var(--qol-accent-ink)!important;stroke-width:1.8!important;stroke-linecap:round!important;stroke-linejoin:round!important;pointer-events:none!important}
            ...
            #${PANEL_ID}{position:fixed!important;right:24px!important;top:74px!important;z-index:1000001!important;display:none!important;flex-direction:column!important;width:min(590px,calc(100vw - 30px))!important;min-width:min(370px,calc(100vw - 30px))!important;min-height:430px!important;max-width:calc(100vw - 16px)!important;max-height:calc(100vh - 16px)!important;border:3px solid var(--qol-border)!important;border-radius:6px!important;background:#f7f5f0!important;box-shadow:0 12px 34px rgba(0,0,0,.48)!important;overflow:hidden!important;resize:both!important;color:#332719!important}
            #${PANEL_ID}.qol-open{display:flex!important}
            #${PANEL_ID} .qol-distance-header{display:flex!important;align-items:center!important;justify-content:space-between!important;flex:0 0 auto!important;min-height:40px!important;padding:8px 10px 8px 12px!important;background:linear-gradient(to bottom,var(--qol-accent-mid),var(--qol-accent-dark))!important;color:#fff!important;cursor:move!important;user-select:none!important;touch-action:none!important}
            ...
            #${PANEL_ID} .qol-distance-swap{display:flex!important;align-items:center!important;justify-content:center!important;align-self:center!important;width:34px!important;height:34px!important;border:1px solid #8e7656!important;border-radius:50%!important;background:#eadfcf!important;color:var(--qol-accent-ink)!important;cursor:pointer!important;font-size:18px!important;font-weight:700!important;user-select:none!important}
            ...
            #${PANEL_ID} .qol-distance-copy,#${PANEL_ID} .qol-distance-send{display:inline-flex!important;align-items:center!important;justify-content:center!important;min-height:29px!important;padding:6px 12px!important;border:1px solid var(--qol-accent-outline)!important;border-radius:3px!important;background:var(--qol-accent)!important;color:#fff!important;box-shadow:0 1px 3px rgba(0,0,0,.18)!important;cursor:pointer!important;user-select:none!important;font-size:9px!important;font-weight:700!important}
            #${PANEL_ID} .qol-distance-copy:hover{background:var(--qol-accent-hover)!important}
  ...
  function updateModifierState() {
    ...
    if (help) help.textContent = merchant ? 'Merchants ignore Tournament Square and hero boots.' : 'Tournament Square and boots apply only after the first 20 fields.';
  }
  function updateResult() {
    ...
    if (plannedArrival) plannedArrival.textContent = sendPlan.valid ? `Landing ${sendPlan.arrivalLabel}` : 'Uses the live server clock';
  }
  ...
  async function copyResult() {
    ...
    const summary = [`(${lastResult.originX}|${lastResult.originY}) → (${lastResult.targetX}|${lastResult.targetY})`, `${lastResult.distance.toFixed(3)} fields`, lastResult.movement.label, `travel ${formatDuration(lastResult.durationSeconds)}`, `arrival ${lastResult.arrival}`];
  ...
  window.APES_DISTANCE_CALCULATOR = Object.freeze({
    ...
    calculateSendPlan: () => {
      const result = calculate();
      return result.valid ? calculateSendPlan(result.durationSeconds) : {
        valid: false
      };
    },
    send: openSendTroops,
