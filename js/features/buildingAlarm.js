function initBuildingAlarm() {
  ...
  function readAlarms() {
    try {
      ...
      return Array.isArray(parsed) ? parsed.filter(alarm => alarm && Number.isFinite(Number(alarm.alarmAt))) : [];
    } catch (_) {
      return [];
    }
  }
  ...
  function getVillageData() {
    const villageElement = document.querySelector('.currentVillageName .selectedItem .villageEntry') || document.querySelector('.villageEntry.active') || document.querySelector('.active .villageEntry') || document.querySelector('.currentVillageName .villageEntry');
  ...
  function matchingAlarmIndex(alarms, village, construction) {
    ...
    return alarms.findIndex(alarm => String(alarm.villageId || '') === String(village.id || '') && Number(alarm.finishAt) === Number(construction.finishAt));
  }
  ...
  function renderAlarmRows(alarms, now, ready) {
    if (!alarms.length) {
      return `<div class="qol-building-alarm-empty">${ready ? 'No buildings are currently available for free instant finish.' : 'No upcoming building alarms are set.'}</div>`;
    }
    return alarms.map(alarm => {
      const transition = getLevelTransition(alarm);
      const relative = ready ? `Finishes in ${formatCountdown(Number(alarm.finishAt) - now)}` : `Alarm in ${formatCountdown(Number(alarm.alarmAt) - now)}`;
      return `
  ...
  function sizeReadySection(list, readyCount) {
    ...
    const visibleRows = Math.max(1, Math.min(Number(readyCount) || 0, READY_MAX_VISIBLE_ROWS));
    ...
    readySection.style.setProperty('overflow-y', readyCount > READY_MAX_VISIBLE_ROWS ? 'auto' : 'hidden', 'important');
  }
  function renderPanel(open = false) {
    ...
    const alarms = readAlarms().filter(alarm => now < Number(alarm.finishAt)).sort((a, b) => Number(a.alarmAt) - Number(b.alarmAt));
    ...
    summary.textContent = alarms.length ? `${alarms.length} active · ${readyAlarms.length} ready` : 'No alarms set';
    list.innerHTML = `
  ...
  function syncClockStates() {
    ...
    document.querySelectorAll('.queueContainer .detailsContent').forEach(contentRow => {
      ...
      clock.title = active ? `Alarm set for ${construction.buildingName} — click to remove` : `Set alarm for ${construction.buildingName} five minutes before completion`;
    });
  }
  ...
  function removeCapturedConstructionAlarm(captured, onlyIfReady = false) {
    ...
    const remaining = alarms.filter(alarm => {
      const matches = String(alarm.villageId || '') === String(captured.villageId || '') && Number(alarm.finishAt) === Number(captured.finishAt);
