(function () {
  ...
  const CATEGORY_CONFIG = {
    helmets: {
      ...
      items: ['Helmet of Regeneration', 'Helmet of Health', 'Helmet of Healing', 'Helmet of the Gladiator', 'Helmet of the Tribune', 'Helmet of the Consul', 'Helmet of the Horseman', 'Helmet of the Cavalry', 'Helmet of the Heavy Cavalry', 'Helmet of the Mercenary', 'Helmet of the Warrior', 'Helmet of the Archon']
    },
    armors: {
      label: 'Armors',
      filterSelector: '.item_category_body_small_flat_black',
      items: ['Armor of Regeneration', 'Armor of Health', 'Armor of Healing', 'Light Scale Armor', 'Scale Armor', 'Heavy Scale Armor', 'Light Breast-plate Armor', 'Breast-plate Armor', 'Heavy Breast-plate Armor', 'Light Chainmail', 'Chainmail', 'Heavy Chainmail']
    },
    leftHand: {
      label: 'Left-hand items',
      filterSelector: '.item_category_leftHand_small_flat_black',
      items: ['Small Map', 'Map', 'Large Map', 'Small Pennant', 'Pennant', 'Great Pennant', 'Small Standard', 'Standard', 'Great Standard', 'Small Spy-glass', 'Spy-glass', 'Great Spy-glass', 'Pouch of the Thief', 'Bag of the Thief', 'Sack of the Thief', 'Small Shield', 'Shield', 'Large Shield', 'Small Horn of the Natarian', 'Horn of the Natarian', 'Huge Horn of the Natarian']
    },
    rightHand: {
      ...
      groups: [{
        label: 'Romans',
        items: ['Short Sword of the Legionnaire', 'Sword of the Legionnaire', 'Long Sword of the Legionnaire', 'Short Sword of the Praetorian', 'Sword of the Praetorian', 'Long Sword of the Praetorian', 'Short Sword of the Imperian', 'Sword of the Imperian', 'Long Sword of the Imperian', 'Short Sword of the Imperatoris', 'Sword of the Imperatoris', 'Long Sword of the Imperatoris', 'Light Lance of the Caesaris', 'Lance of the Caesaris', 'Heavy Lance of the Caesaris']
      }, {
        label: 'Teutons',
        items: ['Club of the Clubswinger', 'Mace of the Clubswinger', 'Morning Star of the Clubswinger', 'Spear of the Spearfighter', 'Spike of the Spearfighter', 'Lance of the Spearfighter', 'Hatchet of the Axeman', 'Axe of the Axeman', 'Battle Axe of the Axeman', 'Light Hammer of the Paladin', 'Hammer of the Paladin', 'Heavy Hammer of the Paladin', 'Short Sword of the Teutonic Knight', 'Sword of the Teutonic Knight', 'Long Sword of the Teutonic Knight']
      }, {
        label: 'Gauls',
        items: ['Spear of the Phalanx', 'Pike of the Phalanx', 'Lance of the Phalanx', 'Short Sword of the Swordsman', 'Sword of the Swordsman', 'Long Sword of the Swordsman', 'Short Bow of the Theutates', 'Bow of the Theutates', 'Long Bow of the Theutates', 'Walking-Staff of the Druidrider', 'Staff of the Druidrider', 'Fighting-Staff of the Druidrider', 'Light Lance of the Haeduan', 'Lance of the Haeduan', 'Heavy Lance of the Haeduan']
      }]
    },
    boots: {
      label: 'Boots',
      filterSelector: '.item_category_shoes_small_flat_black',
      items: ['Boots of Knowledge', 'Boots of Enlightenment', 'Boots of Wisdom', 'Boots of the Mercenary', 'Boots of the Warrior', 'Boots of the Archon', 'Small Spurs', 'Spurs', 'Nasty Spurs', 'Boots of the Chicken']
    },
    mounts: {
      label: 'Mounts',
      filterSelector: '.item_category_horse_small_flat_black',
      items: ['Gelding', 'Thoroughbred', 'Warhorse']
    },
    consumables: {
      label: 'Consumables',
      items: ['Ointment', 'Scroll', 'Bucket', 'Book of Wisdom', 'Artwork', 'Small Bandage', 'Bandage', 'Cage'],
      itemFilterSelectors: {
  ...
  function findAuctionRoot() {
    return document.querySelector('.tabContentBuy.currentTab .auction.buy') || document.querySelector('.tabContentBuy.activeTab .auction.buy') || document.querySelector('.auction.buy');
  }
  function getNativeAuctionTable(auctionRoot) {
    if (!auctionRoot) {
      return null;
    }
    return Array.from(auctionRoot.querySelectorAll('table')).find(table => {
      return !table.closest(`#${PANEL_ID}`) && Boolean(table.querySelector('tbody'));
    }) || null;
  }
  ...
  function populateCategorySelect(select) {
    select.replaceChildren(createPlaceholderOption('Type of item'));
    Object.entries(CATEGORY_CONFIG).forEach(([categoryKey, category]) => {
      const option = document.createElement('option');
  ...
  function populateItemSelect(categoryKey, select) {
    ...
    select.replaceChildren(createPlaceholderOption('Item name'));
    if (!category) {
    ...
    const itemExists = getAllItemsForCategory(category).includes(selectedItem);
  ...
  function handleTypeSelection(typeSelect, itemSelect) {
    ...
    setStatus('Choose the exact item, then click Find all copies.', '');
  }
  function handleItemSelection(typeSelect, itemSelect) {
    ...
    setStatus(`Ready to find every ${selectedItem} listing.`, '');
  }
  function ensurePanel() {
    ...
    const nativeScroll = ensureNativeAuctionScroll(auctionRoot, nativeTable);
  ...
  function setNativeAuctionScrollHeight(auctionRoot, nativeTable, wrapper) {
    ...
    const headerHeight = nativeTable.tHead ? nativeTable.tHead.getBoundingClientRect().height : 0;
    const visibleRowsHeight = rows.slice(0, NATIVE_VISIBLE_ROWS).reduce((height, row) => {
      return height + row.getBoundingClientRect().height;
    }, 0);
    const maximumHeight = Math.ceil(headerHeight + visibleRowsHeight + 1);
    if (maximumHeight > 1) {
  ...
  function ensureNativeAuctionScroll(auctionRoot, nativeTable) {
    ...
    requestAnimationFrame(() => {
      setNativeAuctionScrollHeight(auctionRoot, nativeTable, wrapper);
    });
    return wrapper;
  }
  function restoreNativeAuctionTables() {
    document.querySelectorAll(`.${NATIVE_SCROLL_CLASS}`).forEach(wrapper => {
  ...
  function setInteractionLock(locked) {
    ...
    ['pointerdown', 'pointermove', 'pointerup', 'mousemove', 'mousedown', 'mouseup', 'click', 'dblclick', 'contextmenu', 'wheel'].forEach(eventName => {
  ...
  function getSortedResults() {
    ...
    return results.slice().sort((first, second) => {
      ...
      if (!comparison) {
        comparison = String(first.auctionId).localeCompare(String(second.auctionId), undefined, {
          numeric: true
        });
      }
  ...
  function updateSortHeaders() {
    document.querySelectorAll(`#${PANEL_ID} th[data-sort-key]`).forEach(header => {
      const isActiveSort = header.dataset.sortKey === sortKey;
      const indicator = header.querySelector('.qol-auction-sort-indicator');
      header.setAttribute('aria-sort', isActiveSort ? sortDirection === 'asc' ? 'ascending' : 'descending' : 'none');
      if (indicator) {
        indicator.textContent = isActiveSort ? sortDirection === 'asc' ? '▲' : '▼' : '↕';
      }
    });
  }
  ...
  function isNativeFilterActive(category, itemName, selector = '') {
    const auctionRoot = findAuctionRoot();
    const selectedSelector = selector || getSelectedFilterSelector(category, itemName);
    const filterButton = auctionRoot ? findNativeFilterButton(auctionRoot, selectedSelector) : null;
    return Boolean(filterButton && filterButton.classList.contains('active'));
  }
  ...
  function getRowItemIcon(row) {
    return row.querySelector('td.item i.heroItem[auction-id], ' + 'td.item i.heroItem');
  }
  function getCurrentPageNumber(auctionRoot) {
    const disabledNumbers = Array.from(auctionRoot.querySelectorAll('.tg-pagination li.number.disabled a'));
    const currentPage = disabledNumbers.find(link => {
      return /^\d+$/.test(link.textContent.trim());
    });
    const pageNumber = currentPage ? Number.parseInt(currentPage.textContent.trim(), 10) : Number.NaN;
  ...
  async function waitForAuctionRows(previousRowsSignature, token, options = {}) {
    const {
      requireChange = true,
      allowEmpty = false,
    ...
    while (Date.now() - startedAt < timeout) {
      ...
      if (auctionRoot) {
        ...
        const rowsReady = rows.length > 0 && rows.every(row => {
          const icon = getRowItemIcon(row);
          return Boolean(icon && icon.getAttribute('auction-id'));
        });
        const tableReady = rowsReady || allowEmpty && rows.length === 0;
        const pageChanged = Number.isFinite(previousPage) ? getCurrentPageNumber(auctionRoot) !== previousPage : rowsSignature !== previousRowsSignature;
        const isCandidate = (!requireChange || pageChanged) && tableReady;
        if (isCandidate) {
          if (!readySince) {
            readySince = Date.now();
  ...
  async function clickPaginationControl(control, previousRowsSignature, token) {
    if (!control || isDisabledControl(control)) {
      return false;
    }
    const auctionRoot = findAuctionRoot();
    const previousPage = auctionRoot ? getCurrentPageNumber(auctionRoot) : null;
    control.click();
    return waitForAuctionRows(previousRowsSignature, token, {
      requireChange: true,
      allowEmpty: false,
      previousPage
    });
  }
  async function applyNativeCategoryFilter(category, itemName, token) {
    const auctionRoot = findAuctionRoot();
    if (!auctionRoot) {
    ...
    const filterButton = findNativeFilterButton(auctionRoot, selector);
    if (!filterButton) {
    ...
    if (filterButton.classList.contains('active')) {
      ...
      if (!rowsReady || !isNativeFilterActive(category, itemName, selector)) {
        throw new Error('Travian did not finish applying the selected item filter.');
      }
      return selector;
    }
    ...
    const filterActivated = await waitForCondition(() => {
      ...
      const currentButton = currentRoot ? findNativeFilterButton(currentRoot, selector) : null;
      return Boolean(currentButton && currentButton.classList.contains('active'));
    }, 1600);
    ...
    let rowsReady = await waitForAuctionRows(previousRowsSignature, token, {
      requireChange: true,
      allowEmpty: true
    });
  ...
  async function moveToFirstPage(token) {
    ...
    const firstPage = auctionRoot.querySelector('.tg-pagination .firstPage');
    if (!firstPage || isDisabledControl(firstPage)) {
      return;
    }
    await clickPaginationControl(firstPage, getAuctionRowsSignature(auctionRoot), token);
  }
  function getQuantityText(descriptionCell, itemName) {
    if (!descriptionCell) {
      return '1';
    }
    const fullText = descriptionCell.textContent.replace(/\s+/g, ' ').trim();
    if (!fullText) {
      return '1';
    }
    const itemPosition = fullText.toLowerCase().indexOf(itemName.toLowerCase());
    const remainder = itemPosition >= 0 ? (fullText.slice(0, itemPosition) + fullText.slice(itemPosition + itemName.length)).trim() : '';
  ...
  function readAuctionRow(row, pageNumber) {
    ...
    const endSeconds = countdown ? parseInteger(countdown.getAttribute('countdown')) : 0;
    return {
      auctionId,
      pageNumber,
      itemName,
      ...
      price: parseInteger(row.querySelector('td.price')?.textContent || (cells[4] ? cells[4].textContent : '')),
      itemClasses: Array.from(icon.classList).filter(className => {
        return className === 'heroItem' || className.startsWith('item_category_');
      })
    };
  }
  ...
  function formatRemainingTime(result) {
    ...
    const totalSeconds = Math.max(0, Math.floor((result.endTime - Date.now()) / 1000));
    if (!totalSeconds) {
      return 'Ended';
    }
  ...
  function createResultRow(result) {
    ...
    price.append(document.createTextNode(formatNumber(result.price)), silver);
    priceCell.appendChild(price);
    const actionCell = document.createElement('td');
    const viewButton = document.createElement('div');
    viewButton.className = 'qol-auction-action-btn qol-auction-view-button';
    ...
    row.append(itemCell, quantityCell, timeCell, bidsCell, priceCell, actionCell);
    return row;
  }
  function renderResults() {
    ...
    if (!resultsElement || !titleElement || !countElement || !bodyElement || !emptyElement) {
      return;
    }
    bodyElement.replaceChildren(...getSortedResults().map(createResultRow));
    updateSortHeaders();
    resultsElement.dataset.visible = String(hasScanned);
    titleElement.textContent = selectedItem ? `All ${selectedItem} listings` : 'Matching listings';
    countElement.textContent = `${results.length} ${results.length === 1 ? 'listing' : 'listings'}`;
    emptyElement.dataset.visible = String(hasScanned && results.length === 0);
  }
  function updateRemainingTimes() {
    document.querySelectorAll(`#${PANEL_ID} .qol-auction-time[data-end]`).forEach(element => {
      const result = {
  ...
  async function scanAuctionPages(category, itemName, filterSelector, token) {
    ...
    if (!isNativeFilterActive(category, itemName, filterSelector)) {
      throw new Error('The selected Travian item filter is not active. Scan stopped.');
    }
    await moveToFirstPage(token);
    for (let pageCount = 0; pageCount < MAX_PAGES_TO_SCAN; pageCount += 1) {
      ...
      if (!isNativeFilterActive(category, itemName, filterSelector)) {
        throw new Error('Travian changed the selected item filter. Scan stopped.');
      }
      ...
      const matchingListings = getAuctionRows(auctionRoot).filter(row => {
      ...
      hasScanned = true;
      renderResults();
      setStatus(`Scanned page ${pageNumber}. Found ${foundListings.size} ${foundListings.size === 1 ? 'copy' : 'copies'} so far…`, 'scanning');
      const nextPage = auctionRoot.querySelector('.tg-pagination .nextPage');
      if (!nextPage || isDisabledControl(nextPage)) {
        break;
      }
      const pageChanged = await clickPaginationControl(nextPage, signature, token);
      if (token !== scanToken) {
        return {
          cancelled: true,
          pagesScanned
      ...
      if (!pageChanged) {
        throw new Error('The Auction House did not load the next page.');
      }
    }
    results = Array.from(foundListings.values());
    hasScanned = true;
  ...
  async function startScan() {
    ...
    setBusy(true);
    setStatus(`Opening ${category.label.toLowerCase()}…`, 'scanning');
    try {
      const appliedFilterSelector = await applyNativeCategoryFilter(category, selectedItem, token);
      if (token !== scanToken) {
        return;
      }
      if (!isNativeFilterActive(category, selectedItem, appliedFilterSelector)) {
        throw new Error('The selected Travian item filter is not active. ' + 'Scan was not started.');
      }
      const outcome = await scanAuctionPages(category, selectedItem, appliedFilterSelector, token);
      if (token !== scanToken || outcome.cancelled) {
        return;
      }
      const pageWord = outcome.pagesScanned === 1 ? 'page' : 'pages';
      if (!results.length) {
        setStatus(`No ${selectedItem} is currently listed across ${outcome.pagesScanned} ${pageWord}.`, 'empty');
      } else {
        setStatus(`Found ${results.length} ${results.length === 1 ? 'copy' : 'copies'} across ${outcome.pagesScanned} ${pageWord}. ` + 'They are all displayed together above Travian\'s table.', 'success');
      }
    } catch (error) {
      if (token === scanToken) {
        console.error('[APES Auction House Scanner]', error);
        setStatus(error?.message || 'The scan could not finish. Reopen the Buy tab and try again.', 'error');
      }
    } finally {
      if (token === scanToken) {
        setBusy(false);
  ...
  function findAuctionRowById(auctionRoot, auctionId) {
    return getAuctionRows(auctionRoot).find(row => {
      const icon = getRowItemIcon(row);
      return icon && icon.getAttribute('auction-id') === String(auctionId);
    }) || null;
  }
  function clearNativeAuctionHighlights(auctionRoot = findAuctionRoot()) {
    ...
    auctionRoot.querySelectorAll('tr.qol-auction-native-highlight').forEach(row => {
  ...
  async function openNativeAuction(result) {
    ...
    if (!category || !findAuctionRoot()) {
      setStatus('The Auction House is no longer open. Reopen its Buy tab.', 'error');
      return;
    }
    cancelActiveOperation();
    const token = scanToken;
    setBusy(true);
    setStatus(`Opening auction #${result.auctionId}…`, 'scanning');
    try {
      await applyNativeCategoryFilter(category, selectedItem, token);
      if (token !== scanToken) {
        return;
      }
      await moveToFirstPage(token);
      ...
      for (let pageCount = 0; pageCount < MAX_PAGES_TO_SCAN; pageCount += 1) {
        ...
        visitedSignatures.add(signature);
        const nativeRow = findAuctionRowById(auctionRoot, result.auctionId);
        if (nativeRow) {
          ...
          const focusTarget = nativeRow.querySelector('input.priceInput:not(:disabled)') || nativeRow.querySelector('.bidButtonCol button:not(.disabled)');
          if (focusTarget) {
            setTimeout(() => {
              focusTarget.focus();
            }, 360);
          ...
          setTimeout(() => {
            nativeRow.classList.remove('qol-auction-native-highlight');
          }, 7000);
          setStatus('The selected auction is highlighted in bright ' + 'yellow in Travian\'s table below.', 'success');
          return;
        }
        const nextPage = auctionRoot.querySelector('.tg-pagination .nextPage');
        if (!nextPage || isDisabledControl(nextPage)) {
        ...
        setStatus(`Looking for auction #${result.auctionId} on page ${getCurrentPageNumber(auctionRoot) + 1}…`, 'scanning');
        const pageChanged = await clickPaginationControl(nextPage, signature, token);
        if (!pageChanged) {
          throw new Error('Travian did not load the next page.');
        }
      }
      setStatus(`Auction #${result.auctionId} is no longer available. ` + 'Run a new scan.', 'error');
    } catch (error) {
      if (token === scanToken) {
        console.error('[APES Auction House Scanner]', error);
        setStatus(error?.message || 'The native auction could not be opened.', 'error');
      }
    } finally {
      if (token === scanToken) {
        setBusy(false);
  ...
  function init() {
    ...
    window.addEventListener('qol_setting_changed', handleSettingChange);
    window.addEventListener('hashchange', queueEnsurePanel);
    window.addEventListener('popstate', queueEnsurePanel);
    setInterval(updateRemainingTimes, 1000);
    console.log('[APES Auction House Scanner] In-window item finder initialized.');
  }
