/**
 * APES QoL v2 — Trade Route Optimizer
 *
 * Companion to CP Manager's Trade Route Planner.
 * Reads the planner's celebration-adjusted hourly balances and turns them into
 * concrete source -> destination route suggestions. Route amounts respect the
 * source village's selected x1/x2/x3 frequency and only use villages with a
 * scanned Marketplace as senders.
 *
 * Merchant capacity is intentionally not guessed here. The optimizer balances
 * resources first and explicitly tells the user that merchant capacity still
 * needs to be checked in the Marketplace.
 */
(() => {
    'use strict';
    ...
    function readVillageRows(panel) {
        return Array.from(panel.querySelectorAll('.qol-cp-trade-row')).map(row => {
            ...
            const frequency = Math.max(1, Math.min(3,
                Number.parseInt(row.querySelector('.qol-cp-trade-frequency')?.value || '1', 10) || 1
            ));
    ...
    function signatureFor(rows) {
        return rows.map(row => [
            ...
            row.frequency,
            row.hasMarket ? 1 : 0,
            ...RESOURCE_KEYS.map(key => row.balances[key] ?? 'x')
        ].join('|')).join('||');
    }
    ...
    function buildPlan(rows) {
        ...
        for (const resource of RESOURCE_KEYS) {
            ...
            for (const row of rows) {
                ...
                if (balance > 0) {
                    if (row.hasMarket) {
                        sources.push({ row, remaining: Math.floor(balance) });
                    ...
                    deficits.push({ row, remaining: need });
        ...
        const routes = Array.from(routeMap.values())
            .filter(route => RESOURCE_KEYS.some(key => route.hourly[key] > 0))
            .sort((a, b) => {
    ...
    function optimizerStatusHtml(plan) {
        ...
        const uncovered = RESOURCE_KEYS
            .filter(key => plan.metrics[key].uncovered > 0)
            .map(key => `${formatNumber(plan.metrics[key].uncovered)} ${RESOURCE_LABELS[key]}/h`)
            .join(' · ');

        const stranded = RESOURCE_KEYS
            .filter(key => plan.metrics[key].strandedSurplus > 0)
            .map(key => `${formatNumber(plan.metrics[key].strandedSurplus)} ${RESOURCE_LABELS[key]}/h`)
            .join(' · ');

        return [
            `<strong>${plan.routes.length} suggested route${plan.routes.length === 1 ? '' : 's'}.</strong> Coverage: ${escapeHtml(coverage)}.`,
            uncovered ? `<span class="qol-cp-optimizer-warning">Uncovered: ${escapeHtml(uncovered)}.</span>` : '<span class="qol-cp-optimizer-good">All detected deficits are covered.</span>',
            stranded ? `<span class="qol-cp-optimizer-warning">Surplus without a scanned Market: ${escapeHtml(stranded)}.</span>` : ''
        ].filter(Boolean).join(' ');
    }
    ...
    function planText(plan) {
        ...
        return plan.routes.map((route, index) => {
            const amounts = RESOURCE_KEYS
                .filter(key => route.perRoute[key] > 0)
                .map(key => `${formatNumber(route.perRoute[key])} ${RESOURCE_LABELS[key]}`)
                .join(' | ');
        ...
        const activate = event => {
            ...
            if (copy) {
                ...
                navigator.clipboard?.writeText(text).then(() => {
                    ...
                    window.setTimeout(() => { if (copy.isConnected) copy.textContent = previous; }, 900);
                }).catch(() => {});
                return;
            }
