(() => {
  ...
  function flattenWatchlist() {
    return readWatchlistTabs().flatMap(tab => {
      ...
      return entries.map(entry => ({
        tabId: tab.id,
        tabName: clean(tab.name) || 'Watchlist',
        entry
      }));
    });
  }
  ...
  function archiveFreshness(archive) {
    ...
    return Math.max(Number(archive.updatedAt || 0), 0, ...reports.flatMap(report => [Number(report?.savedAt || 0), Number(report?.updatedAt || 0)]));
  }
  async function readReportArchive() {
    ...
    return selected && typeof selected === 'object' ? {
      ...selected,
      reports: Array.isArray(selected.reports) ? selected.reports : []
    } : {
      reports: []
    };
  }
  ...
  function resolveIdentity(seed) {
    ...
    const ssMembers = currentScans.flatMap(scan => (Array.isArray(scan?.members) ? scan.members : []).map(member => ({
      scan,
      member
    })));
  ...
  function historySocietyMatches(identity) {
    ...
    Object.values(readSocietyHistoryRoot()).forEach(society => {
      ...
      const snapshots = Array.isArray(society.snapshots) ? society.snapshots.slice().sort((a, b) => Number(a.scannedAt || 0) - Number(b.scannedAt || 0)) : [];
      if (!snapshots.length) return;
      const hasPlayer = snapshots.some(snapshot => {
        return (Array.isArray(snapshot?.members) ? snapshot.members : []).some(member => matchIdentity(member?.playerId, member?.name, identity));
      });
      if (hasPlayer) results.push({
        society,
        snapshots
      });
    });
  ...
  function classifyReport(report, identity) {
    ...
    const category = /scout|spy|espion/.test(searchableType) ? 'Scout' : role === 'Attacker' ? 'Attack' : role === 'Defender' ? 'Defense' : 'Other';
    return {
      ...report,
  ...
  function buildTrend(identity, historyMatches, watchMatches) {
    ...
    historyMatches.forEach(group => {
      const points = group.snapshots.map(snapshot => {
        const member = (Array.isArray(snapshot?.members) ? snapshot.members : []).find(item => matchIdentity(item?.playerId, item?.name, identity));
    ...
    const watch = watchMatches.map(item => item.entry).find(entry => Number.isFinite(Number(entry?.populationChange)) || Number.isFinite(Number(entry?.villageChange)));
    if (watch) {
      return {
      ...
      for (let index = 1; index < snapshots.length; index += 1) {
        ...
        if (!previous && current) {
          events.push({
            ...
            type: 'ss-joined',
            title: `Joined ${name}`,
            detail: `Rank ${clean(current.rank) || '—'} · ${formatNumber(numeric(current.population))} population`
          });
          continue;
        }
        if (previous && !current) {
          events.push({
            ...
            type: 'ss-left',
            title: `Left ${name}`,
            detail: `Last seen rank ${clean(previous.rank) || '—'}`
          });
          continue;
        }
        ...
        const detail = [populationDelta ? `Population ${formatSigned(populationDelta)}` : '', villageDelta ? `Villages ${formatSigned(villageDelta)}` : '', rankChanged ? `Rank ${rankBefore} → ${rankAfter}` : ''].filter(Boolean).join(' · ');
        events.push({
          timestamp,
          type: 'ss-change',
          title: `${name} update`,
  ...
  function buildTimeline(identity, watch, history, reports) {
    ...
    watch.forEach(item => {
      ...
      if (Number(entry.lastUpdatedAt || 0)) {
        const changes = [Number.isFinite(Number(entry.populationChange)) ? `Population ${formatSigned(entry.populationChange)}` : '', Number.isFinite(Number(entry.villageChange)) ? `Villages ${formatSigned(entry.villageChange)}` : ''].filter(Boolean).join(' · ');
        events.push({
    ...
    return events.filter(event => Number.isFinite(Number(event.timestamp)) && Number(event.timestamp) > 0).sort((a, b) => b.timestamp - a.timestamp).slice(0, MAX_TIMELINE_ROWS);
  }
...
`;
  ...
  function renderHero(model) {
    ...
    const tagHtml = [...model.tags.map(tag => badge(tag, 'role')), ...model.watch.map(item => badge(item.tabName, 'neutral')).slice(0, 4)].join('');
  ...
  function reportSummary(model) {
    const groups = [['Attacking', model.reports.filter(report => report.role === 'Attacker').length], ['Defending', model.reports.filter(report => report.role === 'Defender').length], ['Scout', model.reports.filter(report => report.category === 'Scout').length]];
  ...
  function renderTimeline(model) {
    ...
    target.innerHTML = model.timeline.length ? `<div class="qol-pd-timeline">${model.timeline.map(event => `
                ...
                </div>`).join('')}</div>` : '<div class="qol-pd-empty">No dated Watchlist, Secret Society or Report Archive activity is available for this player yet.</div>';
  }
  ...
  function inlineOpenButton(playerId, playerName, className = '') {
    ...
    const activate = event => {
      ...
      void open({
        playerId: button.dataset.pdPlayerId,
        playerName: button.dataset.pdPlayerName
      });
    };
  ...
  function ssNameMap() {
    ...
    readCurrentSocieties().forEach(scan => {
      (Array.isArray(scan?.members) ? scan.members : []).forEach(member => {
        ...
        map.set(name, {
          playerId: clean(member?.playerId),
  ...
  function injectSocietyButtons() {
    ...
    document.querySelectorAll('#qol-ss-scanner-panel .qol-ss-name-column').forEach(cell => {
      ...
      const identity = map.get(normalized(name)) || {
        playerName: name,
        playerId: ''
      };
  ...
  function profilePlayerName() {
    const selectors = ['div.content[ng-if="!kingdomProfile"]', '.playerProfile .playerName', '.playerName'];
  ...
  window.APES_PLAYER_DOSSIER = Object.freeze({
    open,
    close,
