(function initResourceCapacityTimer() {
  ...
  let intervalId = null;
  function isEnabled() {
    return typeof window.isQolEnabled === 'function' ? window.isQolEnabled(FEATURE_KEY) : localStorage.getItem(`qol_${FEATURE_KEY}`) !== 'false';
  }
  ...
  function readResource(resource) {
    ...
    const current = parseUnsignedInteger(progressbar.getAttribute('value')) ?? parseUnsignedInteger(amountNode?.textContent);
    const capacity = parseUnsignedInteger(progressbar.getAttribute('max-value')) ?? parseUnsignedInteger(capacityNode?.textContent);
