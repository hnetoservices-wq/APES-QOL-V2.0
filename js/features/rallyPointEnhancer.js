function initRallyPointEnhancer() {
  ...
  function getMovementCategory(value) {
    ...
    if (normalized.includes('reinforcement') || normalized.includes('reinforcements') || normalized.includes('support')) {
  ...
  function getSelectedMovementTypes() {
    ...
    document.querySelectorAll('[data-qol-rally-movement-type]').forEach(checkbox => {
      const type = checkbox.getAttribute('data-qol-rally-movement-type');
      if (type && Object.prototype.hasOwnProperty.call(selectedTypes, type)) {
        selectedTypes[type] = checkbox.checked === true;
      }
    });
  ...
  function shouldIncludeMovement(value) {
    ...
    const selectedTypes = activeMovementTypes || getSelectedMovementTypes();
  ...
  function updateResultCount() {
    ...
    compiledWaves.forEach(wave => {
      const category = getMovementCategory(wave.type);
      if (category) {
        counts[category] += 1;
      }
    });
    ...
    const detailParts = [['attack', 'attack', 'attacks'], ['siege', 'siege', 'sieges'], ['raid', 'raid', 'raids'], ['reinforcement', 'reinforcement', 'reinforcements']].filter(([type]) => counts[type] > 0).map(([type, singular, plural]) => {
      const count = counts[type];
      return `${count} ${count === 1 ? singular : plural}`;
    });
    countElement.textContent = `${total} ${total === 1 ? 'movement' : 'movements'}` + (detailParts.length ? ` · ${detailParts.join(' · ')}` : '');
  }
  ...
  function renderEmptyState(container, title = 'No scan results yet.', description = 'Choose the incoming types, then select “Scan Incomings”.') {
    container.innerHTML = `
  ...
  console.log('[RallyPointEnhancer] Module successfully initialized (V5.0).');
  window.addEventListener('qol_close_others', event => {
    if (event.detail && event.detail.source !== 'wm') {
  ...
  function getContextData() {
    ...
    if (playerElement && playerElement.textContent.trim()) {
      ...
      const fallbackPlayer = document.querySelector('.playerName') || document.querySelector('.avatar .name');
      if (fallbackPlayer) {
        playerName = fallbackPlayer.textContent.replace(/[\r\n]+/g, ' ').trim();
      }
    }
    ...
    if (villageElement && villageElement.textContent.trim()) {
      ...
      const fallbackVillage = document.querySelector('.villageList .active .name') || document.querySelector('.villageName');
      if (fallbackVillage) {
        villageName = fallbackVillage.textContent.replace(/[\r\n]+/g, ' ').trim();
      }
    }
    return {
      playerName,
  ...
  function getFallbackVillageCopyHeader() {
    ...
    const villageName = window.APES?.context?.getVillageName?.() || context.villageName || 'Current Village';
    const villageElement = document.querySelector('.currentVillageName .villageEntry, ' + '.villageEntry.active, ' + '.active .villageEntry');
    const coordinateMatch = String(villageElement?.textContent || '').match(/\((-?\d+)\s*\|\s*(-?\d+)\)/);
    return coordinateMatch ? `${villageName} - (${coordinateMatch[1]}|${coordinateMatch[2]})` : villageName;
  }
  function getCurrentVillageCopyHeader() {
    return new Promise(resolve => {
      ...
      const handleMessage = event => {
        ...
        const village = villages.find(item => String(item?.villageId || '') === activeId) || villages.find(item => item?.isActive === true);
        if (!village) {
        ...
        finish(Number.isFinite(x) && Number.isFinite(y) ? `${name} - (${x}|${y})` : name);
      };
      ...
      window.setTimeout(() => finish(getFallbackVillageCopyHeader()), 400);
    });
  }
  function getRallyPointContainer() {
    ...
    const elements = Array.from(document.querySelectorAll('div, span, h1, h2, h3, th'));
    const inboundHeader = elements.find(element => {
      return element.textContent && element.textContent.includes('Inbound troops');
    });
    if (inboundHeader) {
      return inboundHeader.closest('.window, ' + '.buildingView, ' + 'div[style*="position"]') || inboundHeader.parentElement;
    }
    return null;
  }
  ...
  function scrapePageData(container, contextData) {
    ...
    if (rowElements.length > 0) {
      rowElements.forEach(row => {
        const typeElement = row.querySelector('.type, ' + '.movementType, ' + '.title');
        const enemyElement = row.querySelector('.player, ' + '.playerName, ' + '.sourcePlayer');
        const villageElement = row.querySelector('.village, ' + '.villageName, ' + '.sourceVillage');
        const travelElement = row.querySelector('.timer, ' + '.duration, ' + '.remaining');
        const landingElement = row.querySelector('.landingTime, ' + '.at, ' + '.time');
        if (enemyElement && landingElement) {
          const movementType = (typeElement ? typeElement.textContent : 'Attack').trim();
          if (shouldIncludeMovement(movementType)) {
            pageWaves.push({
              enemy: (enemyElement.textContent || 'Unknown').trim(),
              enemyVillage: (villageElement ? villageElement.textContent : 'Unknown').trim(),
              type: movementType,
              travel: (travelElement ? travelElement.textContent : '00:00:00').trim(),
              landing: landingElement.textContent.trim(),
              player: contextData.playerName,
              village: contextData.villageName
            });
          }
        }
      });
    ...
    detailRows.forEach(row => {
      const movementIcon = row.querySelector('.troopsTitle [class*="movement_"][class*="_small"]');
      if (!movementIcon) {
        return;
      }
      const iconClasses = String(movementIcon.className || '');
      const titleText = String(row.querySelector('.troopsTitle')?.textContent || '');
      const category = getMovementCategory(titleText) || getMovementCategory(iconClasses);
      if (!category || !(activeMovementTypes || getSelectedMovementTypes())[category]) {
        return;
      }
      const enemyElement = row.querySelector('.troopsTitle .playerLink');
      const villageElement = row.querySelector('.troopsTitle .villageLink');
      const travelElement = row.querySelector('.countdownTo [countdown], ' + '.countdownContainer [countdown]');
      const landingElement = row.querySelector('.countdownTo [i18ndt], ' + '.countdownContainer [i18ndt]');
      if (!enemyElement || !villageElement || !landingElement) {
        return;
      }
      ...
      pageWaves.push({
        enemy: enemyElement.textContent.trim() || 'Unknown',
        enemyVillage: villageElement.textContent.trim() || 'Unknown',
        type: movementLabels[category],
        travel: travelElement ? travelElement.textContent.trim() : '00:00:00',
        landing: landingElement.textContent.trim(),
        player: contextData.playerName,
        village: contextData.villageName
      });
    ...
    for (const match of matches) {
      ...
      if (shouldIncludeMovement(trimmedType)) {
        pageWaves.push({
          enemy: enemy.trim(),
          enemyVillage: enemyVillage.trim(),
          type: trimmedType,
          travel: travel.trim(),
  ...
  function renderScrollableUI(container) {
    ...
    if (compiledWaves.length === 0) {
      renderEmptyState(container, 'No matching incomings found.', 'The Rally Point scan completed without finding any of the selected movement types.');
      updateResultCount();
      return;
    }
    const rows = compiledWaves.map(wave => {
      const typeText = String(wave.type || 'Attack');
      const category = getMovementCategory(typeText) || 'attack';
      return `
                        <tr>
                            <td class="qol-rp-enemy">
                                ${escapeHtml(wave.enemy || 'Unknown')}
                            </td>

                            <td>
                                ${escapeHtml(wave.enemyVillage || 'Unknown')}
                            </td>
                            ...
                            <td class="qol-rp-time">
                                ${escapeHtml(wave.travel || '00:00:00')}
                            </td>

                            <td class="qol-rp-landing">
                                ${escapeHtml(wave.landing || 'Unknown')}
                            </td>
                        </tr>
                    `;
    }).join('');
  ...
  async function copyResultsToClipboard() {
    if (!compiledWaves.length) {
      setStatus('There are no scan results to copy.', 'error');
      return;
    }
    const movementOutput = compiledWaves.map(wave => {
      return `${wave.type} by ` + `${wave.enemy} from ` + `${wave.enemyVillage} ` + `in ${wave.travel} ` + `at ${wave.landing}`;
    }).join('\n');
    const villageHeader = await getCurrentVillageCopyHeader();
    const output = `${villageHeader}\n${movementOutput}`;
    try {
      await navigator.clipboard.writeText(output);
      const copyButton = document.getElementById('qol-btn-copy');
      if (copyButton) {
      ...
      setStatus(`Copied ${compiledWaves.length} incoming movements.`, 'success');
    } catch (error) {
      console.error('[RallyPointEnhancer] Clipboard access error:', error);
      setStatus('Could not copy the scan results.', 'error');
    }
  }
  function clearScan() {
    if (isScanning) {
    ...
    const tableTarget = document.getElementById('qol-table-target-wrapper');
    const copyButton = document.getElementById('qol-btn-copy');
    const clearButton = document.getElementById('qol-btn-clear');
    const parseButton = document.getElementById('qol-btn-merge');
    if (tableTarget) {
      renderEmptyState(tableTarget);
    }
    if (copyButton) {
  ...
  function mountPanel() {
    ...
    const header = bar.querySelector('.qol-rp-header');
    const closeButton = bar.querySelector('#qol-rp-close');
    const parseButton = bar.querySelector('#qol-btn-merge');
    const copyButton = bar.querySelector('#qol-btn-copy');
    const clearButton = bar.querySelector('#qol-btn-clear');
    const tableTarget = bar.querySelector('#qol-table-target-wrapper');
    ...
    parseButton.addEventListener('click', async event => {
      ...
      activeMovementTypes = getSelectedMovementTypes();
      if (!Object.values(activeMovementTypes).some(Boolean)) {
        activeMovementTypes = null;
        setStatus('Select at least one incoming movement type.', 'error');
        return;
      }
      isScanning = true;
      compiledWaves = [];
      ...
      setButtonDisabled(copyButton, true);
      ...
      updateResultCount();
      renderScanningState(tableTarget);
      ...
      try {
        ...
        console.error('[RallyPointEnhancer] Rally Point render error:', error);
      }
      if (!panelLoaded) {
        isScanning = false;
        activeMovementTypes = null;
        ...
        renderEmptyState(tableTarget, 'Rally Point could not be opened.', 'Open the Rally Point manually and try the scan again.');
        return;
      }
      parseButton.textContent = 'Scanning...';
      setStatus('Scanning incoming movement pages...', 'working');
      updateScanLock('Scanning incoming movement pages...');
      collectAllPages(document.getElementById('qol-merge-status'), () => {
        isScanning = false;
        renderScrollableUI(tableTarget);
        activeMovementTypes = null;
        ...
        if (compiledWaves.length > 0) {
          copyButton.style.setProperty('display', 'inline-flex', 'important');
          setStatus(`Scan complete. Found ${compiledWaves.length} incoming movements.`, 'success');
        } else {
          copyButton.style.setProperty('display', 'none', 'important');
          setStatus('Scan complete. No matching incomings found.', 'success');
        }
        hideScanLock();
      }).catch(error => {
  ...
  function mountToggleButton() {
    ...
    toggleButton.setAttribute('title', 'Rally Point Scanner');
