(function initNpcCalculatorModule() {
  ...
  const RESOURCE_META = {
    wood: {
      label: 'Wood',
      icon: 'unit_wood_small_illu resType1',
      storage: 'Warehouse',
      fallback: 1
    },
    clay: {
      label: 'Clay',
      icon: 'unit_clay_small_illu resType2',
      storage: 'Warehouse',
      fallback: 2
    },
    iron: {
      label: 'Iron',
      icon: 'unit_iron_small_illu resType3',
      storage: 'Warehouse',
      fallback: 3
    },
    crop: {
      label: 'Crop',
      icon: 'unit_crop_small_illu resType4',
      storage: 'Granary',
      fallback: 4
    }
  };
  ...
  function parseResourceText(text) {
    ...
    if (!slash) return {
      current: parseInteger(cleaned),
      capacity: null
    };
    return {
      current: parseInteger(slash[1]),
      capacity: parseInteger(slash[2])
    };
  }
  function readResource(key) {
    const meta = RESOURCE_META[key];
    const stock = document.querySelector(`#resourceBar .stockContainer.${key}`) || document.querySelector(`.stockContainer.${key}`);
    ...
    const current = parseInteger(progressbar?.getAttribute('value')) ?? parseInteger(directText(amountNode)) ?? parseInteger(amountNode?.textContent) ?? stockText.current ?? fallbackText.current ?? 0;
    const capacity = parseInteger(progressbar?.getAttribute('max-value')) ?? parseInteger(progressbar?.getAttribute('max')) ?? parseInteger(directText(capacityNode)) ?? parseInteger(capacityNode?.textContent) ?? stockText.capacity ?? fallbackText.capacity;
    return {
  ...
  function getVillageResources() {
    ...
    return {
      ...values,
      total: RESOURCE_KEYS.reduce((sum, key) => sum + values[key].current, 0)
    };
  }
  ...
  function buildResourceCards(prefix, stockMode) {
    return RESOURCE_KEYS.map(key => {
      const meta = stockMode ? `<div class="qol-npc-resource-meta"><span id="${prefix}-${key}-capacity">Cap —</span><span id="${prefix}-${key}-percent">—</span></div>` : `<div class="qol-npc-resource-meta"><span id="${prefix}-${key}-required">Need 0</span><span id="${prefix}-${key}-overflow">Leaves 0</span></div>`;
...
`;
  ...
  function capacitiesFor(resources) {
    return Object.fromEntries(RESOURCE_KEYS.map(key => [key, Number.isFinite(resources[key].capacity) && resources[key].capacity > 0 ? resources[key].capacity : Number.POSITIVE_INFINITY]));
  }
  function runGreedyPasses(entries, resources, order) {
    ...
    return {
      passes,
      impossible: remaining.some(count => count > 0)
    };
  }
  ...
  function buildExecutionPlan(entries, resources) {
    ...
    if (impossibleUnit) return {
      passes: [],
      impossible: true,
      impossibleUnit: impossibleUnit.label
    };
    const candidates = buildCandidateOrders(positiveEntries, resources).map(order => runGreedyPasses(positiveEntries, resources, order)).filter(result => !result.impossible);
