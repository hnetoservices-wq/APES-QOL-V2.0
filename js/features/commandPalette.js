(() => {
  ...
  const RADIAL_FEATURES = [{
    ...
    icon: ICONS.alarm,
    run: () => APES.ui.showById('qol-building-alarm-panel', 'block')
  }, {
    id: 'igm.open',
  ...
  function featureEnabled(featureKey) {
    return typeof window.isQolEnabled !== 'function' || window.isQolEnabled(featureKey) === true;
  }
  function navigateTo(suffix) {
    const currentPath = String(window.location.hash || '').replace(/^#\/?/, '');
    const basePath = currentPath.replace(/\/(?:window|herotab):.*$/i, '').replace(/\/$/, '') || 'page:village';
    closeRadial();
  ...
  function getRingRadius(itemCount, ringIndex) {
    const minimumRadius = itemCount > 1 ? MIN_ITEM_DISTANCE / (2 * Math.sin(Math.PI / itemCount)) : 0;
  ...
  function getPosition(index, features) {
    ...
    const ringItems = features.slice(ringStart, ringStart + ITEMS_PER_RING);
  ...
  function renderRadial() {
    ...
    const rings = Math.max(1, Math.ceil(features.length / ITEMS_PER_RING));
    const lastRingStart = (rings - 1) * ITEMS_PER_RING;
    const lastRingCount = Math.min(ITEMS_PER_RING, Math.max(1, features.length - lastRingStart));
  ...
  function commandPaletteEnabled() {
    try {
      return localStorage.getItem('qol_keybind_commandPalette') !== 'false';
    } catch (_error) {
      return true;
    }
  }
  function isCommandKey(event) {
    return event.code === 'KeyG' || String(event.key || '').toLowerCase() === 'g';
  }
  ...
  window.addEventListener('keydown', event => {
    if (!isCommandKey(event) || !commandPaletteEnabled() || event.ctrlKey || event.altKey || event.metaKey || event.shiftKey || APES.ui.isTypingTarget(event.target)) {
      return;
    }
