(function initCpManagerModule() {
  ...
  function isEnabled() {
    return typeof window.isQolEnabled === 'function' ? window.isQolEnabled(FEATURE_KEY) === true : localStorage.getItem(`qol_${FEATURE_KEY}`) !== 'false';
  }
  ...
  function detectServerSpeed(result) {
    ...
    for (const village of result?.townHalls?.villages || []) {
      ...
      for (const event of village.allCelebrations || []) {
        ...
        if (Math.abs(ratio - 3) < .12) return {
          speed: 3,
          source: 'celebration timing'
        };
        if (Math.abs(ratio - 1) < .12) return {
          speed: 1,
          source: 'celebration timing'
        };
      }
...
`;
  ...
  function readTownState() {
    ...
    if (cultureCell) {
      ...
      const candidates = Array.from(cultureCell.querySelectorAll('span')).filter(element => !element.classList.contains('currentValue')).map(element => parseInteger(element.textContent)).filter(Number.isFinite);
  ...
  function getCulturePointsTable() {
    return document.querySelector('.loadedTab.tabCulturePoints.currentTab .cpOverview table.villagesTable') || document.querySelector('.loadedTab.tabCulturePoints.activeTab .cpOverview table.villagesTable') || document.querySelector('.cpOverview table.villagesTable');
  }
  ...
  function getCelebrationType(card) {
    ...
    if (image?.classList.contains('celebration_small_illu') || /small/i.test(title)) return {
      type: 'small',
      reward: reward || SMALL_CELEBRATION_CAP
    };
    if (image?.classList.contains('celebration_large_illu') || /(large|big)/i.test(title)) return {
      type: 'big',
      reward: reward || BIG_CELEBRATION_CAP
    };
    return null;
  }
  ...
  function getArtworkReward(result) {
    return Math.min(ARTWORK_CAP, Math.max(0, Number(result?.cpPerDay || 0)));
  }
  function getPlannerArtworkCount() {
    const value = Number.parseInt(document.querySelector(`#${PLANNER_ID} .qol-cp-artwork-input`)?.value || '0', 10);
    return Number.isFinite(value) && value > 0 ? Math.min(365, value) : 0;
  }
  function buildArtworkEvents(result, count, now = Date.now()) {
    const reward = getArtworkReward(result);
    return reward > 0 && count > 0 ? Array.from({
      length: count
    }, (_, index) => ({
  ...
  function getPlannerPeriodDays() {
    const value = Number.parseFloat(document.querySelector(`#${PLANNER_ID} .qol-cp-period-input`)?.value || '1');
    return Number.isFinite(value) && value > 0 ? Math.min(365, Math.max(.25, value)) : 1;
  }
  function getPlannerTargetSlot() {
    const value = Number.parseInt(document.querySelector(`#${PLANNER_ID} .qol-cp-target-select`)?.value || '', 10);
    return Number.isFinite(value) && value >= 1 && value <= CP_SLOT_TARGETS.length ? value : lastScanResult ? getNextExpansionSlot(lastScanResult) : null;
  }
  ...
  function buildPlannerPrediction(result, plans, periodDays, targetCp = result.target, artworkCount = 0) {
    ...
    result.townHalls.celebrationEvents.forEach(event => {
      if (event.startMs > result.readAtMs && event.startMs <= now) cp += event.reward;
    });
    if (cp >= target) return formatPredictionResult(now, []);
    const fixed = [...result.townHalls.celebrationEvents.filter(event => event.startMs > now).map(event => ({
      ...event,
      source: 'queued'
    })), ...buildArtworkEvents(result, artworkCount, now)].sort((a, b) => a.startMs - b.startMs);
    const sequences = plans.filter(plan => plan.level > 0 && plan.durationSeconds > 0 && plan.reward > 0).map(plan => ({
      ...plan,
    ...
    for (let guard = 0; guard < 10000; guard += 1) {
      ...
      if (!next) return rate <= 0 ? {
        text: 'Planner ETA unavailable',
        targetDate: null,
        exactMinutes: null
      } : formatPredictionResult(cursor + (target - cp) / rate, applied);
      ...
      if (next.source === 'queued' || next.source === 'artwork') fixed.shift();else if (sequence) {
        if (sequence.run247) {
    ...
    return {
      text: 'Planner ETA exceeded calculation range',
      targetDate: null,
      exactMinutes: null
    };
  }
  ...
  function mountPanel() {
    ...
    panel.querySelector('.qol-cp-plan-btn').addEventListener('click', event => {
      event.stopPropagation();
      togglePlanner();
    });
