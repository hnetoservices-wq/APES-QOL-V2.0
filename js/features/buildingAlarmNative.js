/**
 * APES QoL — Building Alarm (native clock version)
 *
 * Travian's own symbol_clock_small_flat_black construction clock is the alarm
 * control. Nothing is replaced: header hammers and native clock geometry stay
 * untouched. Red = no alarm, green = alarm set.
 */
function initBuildingAlarmNative() {
    'use strict';
    ...
    const STYLE_ID = 'qol-building-alarm-native-styles';
    ...
    function getLevelTransition(alarm) {
        ...
        if (levels.length === 1) return { from: name, to: `${name} ${levels[0]}` };
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            #${PANEL_ID}{position:fixed!important;right:18px!important;top:120px!important;z-index:1000000!important;display:none;width:560px!important;max-width:calc(100vw - 24px)!important;max-height:calc(100vh - 140px)!important;border:2px solid var(--qol-border)!important;border-radius:5px!important;background:#f7f5f0!important;box-shadow:0 10px 28px rgba(0,0,0,.4)!important;overflow:hidden!important;color:#3e3021!important}
            ...
            .qol-building-alarm-list{display:flex!important;flex-direction:column!important;max-height:340px!important;padding:10px!important;gap:7px!important;overflow-y:auto!important}
            .qol-building-alarm-row{display:grid!important;grid-template-columns:minmax(90px,1fr) minmax(180px,1.7fr) minmax(105px,.9fr) 24px!important;align-items:center!important;gap:8px!important;min-height:45px!important;padding:7px 8px!important;border:1px solid #ded4c6!important;border-radius:4px!important;background:#fff!important}
            .qol-building-alarm-row.ready{background:#eef6e7!important;border-color:#bed3aa!important}.qol-building-alarm-village{overflow:hidden!important;color:#5a4227!important;font-size:10px!important;font-weight:bold!important;text-overflow:ellipsis!important;white-space:nowrap!important;cursor:pointer!important}.qol-building-alarm-building{color:#4a3b2a!important;font-size:10px!important}.qol-building-alarm-time{text-align:center!important;color:#6b5942!important;font-family:Consolas,monospace!important;font-size:9px!important}.qol-building-alarm-row.ready .qol-building-alarm-time{color:#416923!important;font-weight:bold!important}.qol-building-alarm-remove{display:inline-flex!important;align-items:center!important;justify-content:center!important;width:22px!important;height:22px!important;border-radius:3px!important;color:#9a3b32!important;font-size:17px!important;cursor:pointer!important}.qol-building-alarm-remove:hover{background:#f5dfdc!important}.qol-building-alarm-empty{padding:18px 12px!important;color:#83725d!important;font-size:10px!important;text-align:center!important}
        `;
    ...
    function makeDraggable(panel, handle) {
        ...
        handle.addEventListener('pointerdown', event => {
            ...
            const move = moveEvent => {
                const left = Math.max(8, Math.min(moveEvent.clientX - offsetX, window.innerWidth - panel.offsetWidth - 8));
                const top = Math.max(8, Math.min(moveEvent.clientY - offsetY, window.innerHeight - panel.offsetHeight - 8));
    ...
    function mountPanel() {
        ...
        panel.innerHTML = `
            <div class="qol-building-alarm-header">
                <span class="qol-building-alarm-heading"><strong>Building Alarms</strong><span class="qol-building-alarm-summary">No alarms set</span></span>
        ...
        close.addEventListener('click', () => panel.style.setProperty('display', 'none', 'important'));
        return panel;
    }
    ...
    function renderPanel(open = false) {
        ...
        summary.textContent = alarms.length ? `${alarms.length} active` : 'No alarms set';
        ...
        if (!alarms.length) {
            ...
            list.innerHTML = alarms.map(alarm => {
                ...
                const ready = alarm.triggered === true || now >= Number(alarm.alarmAt);
                ...
                return `
                    <div class="qol-building-alarm-row${ready ? ' ready' : ''}" data-alarm-id="${escapeHtml(alarm.id)}">
                        <span class="qol-building-alarm-village" data-village-id="${escapeHtml(alarm.villageId)}">${escapeHtml(alarm.villageName)}</span>
                        <span class="qol-building-alarm-building">${escapeHtml(transition.from)} → ${escapeHtml(transition.to)}</span>
                        <span class="qol-building-alarm-time">${ready ? 'Ready' : escapeHtml(formatServerTime(alarm.alarmAt))}<br>${escapeHtml(remaining)}</span>
    ...
    function removeConstructionAlarm(contentRow, onlyIfReady = false) {
        const construction = getConstructionData(contentRow);
        if (!construction) return;
        const village = getVillageData();
        const now = getServerTimestamp();
    ...
    function handleClick(event) {
        ...
        if (!getConstructionData(contentRow)) return;
        window.setTimeout(
            () => removeConstructionAlarm(contentRow, Boolean(finishButton)),
            600
        );
    }
