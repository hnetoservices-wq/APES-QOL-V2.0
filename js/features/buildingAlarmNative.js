 ...
 *
 * Travian's own construction clock is the alarm control. Nothing is replaced:
 * header hammers and native clock geometry remain untouched.
...
function initBuildingAlarmNative() {
    ...
    function injectStyles() {
        ...
        style.textContent = `
            ...
            .qol-building-alarm-list{display:flex!important;flex-direction:column!important;max-height:390px!important;padding:10px!important;gap:10px!important;overflow-y:auto!important}
    ...
    function makeDraggable(panel, handle) {
        ...
        handle.addEventListener('pointerdown', event => {
            ...
            const move = moveEvent => {
                const left = Math.max(
                    8,
                    Math.min(moveEvent.clientX - offsetX, window.innerWidth - panel.offsetWidth - 8)
                );
                const top = Math.max(
                    8,
                    Math.min(moveEvent.clientY - offsetY, window.innerHeight - panel.offsetHeight - 8)
                );
    ...
    function toggleAlarmForRow(contentRow) {
        ...
        // Restored behavior: creating an alarm immediately opens the manager.
        renderPanel(added);
    }
