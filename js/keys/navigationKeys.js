function navigateToRoute(routeParam) {
  ...
  if (existingIndex !== -1) parts[existingIndex] = routeParam;else parts.push(routeParam);
  window.location.hash = `#/${parts.filter(Boolean).join('/')}`;
}
function openVillageWindow(windowName, beforeWindowParams = [], afterWindowParams = []) {
  const currentParts = window.location.hash.startsWith('#/') ? window.location.hash.substring(2).split('/').filter(Boolean) : [];
...
function removeRouteParam(targetWindowName) {
  ...
  parts = parts.filter(part => {
    return part !== `window:${targetWindowName}` && !part.startsWith('location:') && !part.startsWith('cp:') && !part.startsWith('herotab:') && !part.startsWith('tab:') && !part.startsWith('subtab:');
  });
  window.location.hash = `#/${parts.filter(Boolean).join('/')}`;
}
...
function changeVillage(direction) {
  const navigationButtons = Array.from(document.querySelectorAll(`#villageList .navigation.${direction}`));
...
function isVisibleNavigationTarget(element) {
  ...
  return style.display !== 'none' && style.visibility !== 'hidden' && Number.parseFloat(style.opacity || '1') > 0 && bounds.width > 0 && bounds.height > 0;
}
...
function findActiveBuildingQueueSlot() {
  const selectors = ['.queueContainer.usedSlot.queueType1', '.queueContainer.usedSlot'];
  for (const selector of selectors) {
    const visibleSlot = Array.from(document.querySelectorAll(selector)).find(isVisibleNavigationTarget);
    if (visibleSlot) return visibleSlot;
  }
  return null;
}
function describeInstantFinishCandidate(element) {
  const values = [element.id, element.className, element.textContent, element.getAttribute('title'), element.getAttribute('tooltip'), element.getAttribute('tooltip-translate'), element.getAttribute('aria-label'), element.getAttribute('clickable'), element.getAttribute('ng-click'), element.getAttribute('on-click'), element.getAttribute('on-pointer-up'), element.getAttribute('on-pointer-down')];
  const child = element.querySelector?.('i,img,span');
  if (child) {
    values.push(child.id, child.className, child.getAttribute?.('title'), child.getAttribute?.('tooltip'), child.getAttribute?.('tooltip-translate'), child.getAttribute?.('aria-label'));
  }
...
function findInstantFinishButton(slot) {
  const scope = slot.closest('[ng-include]') || slot.closest('.buildingQueue') || slot.parentElement || document.body;
  const selector = ['[clickable]', '[ng-click]', '[on-click]', '[on-pointer-up]', '[on-pointer-down]', 'button', 'a', '[role="button"]'].join(',');
  const slotBounds = slot.getBoundingClientRect();
  const candidates = Array.from(scope.querySelectorAll(selector)).filter(element => element !== slot).map(element => ({
    element,
  ...
  const positionalMatch = candidates.find(candidate => {
    ...
    return candidate.score >= 35 && bounds.left >= slotBounds.left + slotBounds.width * 0.55 && !/(cancel|abort|remove|delete|destroy|stop|close|dismiss|symbol_x|icon_x)/i.test(description);
  });
...
function findHoveredInstantFinishBubble() {
  return Array.from(document.querySelectorAll('.buildingBubble.clickable:hover')).find(bubble => isVisibleNavigationTarget(bubble) && !bubble.classList.contains('disabled') && !bubble.querySelector('.colorLayer.notAtAll')) || null;
}
...
function handleNavigation(code) {
  ...
  if (modifiersActive || legacyModifierEvent && (legacyModifierEvent.ctrlKey || legacyModifierEvent.altKey || legacyModifierEvent.metaKey || legacyModifierEvent.shiftKey)) {
    return;
  }
  ...
  if (activeElement) {
    ...
    if (activeElement.closest('.chat') || activeElement.closest('.message') || activeElement.closest('.input')) {
      return;
    }
  }
  ...
  switch (code) {
    ...
    case 'KeyT':
      if (isKeybindEnabled('rallyPoint')) {
        const rallyPointIsOpen = currentHash.includes('window:building') && currentHash.includes('location:32') && !currentHash.includes('tab:FarmList');
        if (rallyPointIsOpen) {
    ...
    case 'KeyC':
      if (isKeybindEnabled('conversations')) {
        if (currentHash.includes('window:igm')) removeRouteParam('igm');else {
    ...
    case 'KeyV':
      if (isKeybindEnabled('villagesOverview')) {
        if (currentHash.includes('window:villagesOverview')) removeRouteParam('villagesOverview');else navigateToRoute('window:villagesOverview');
      }
      break;
  }
}
