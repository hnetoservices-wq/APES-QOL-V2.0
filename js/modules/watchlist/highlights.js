(() => {
  ...
  const CANDIDATE_SELECTOR = ['a', '[data-player-id]', '[data-playerid]', '[href*="playerId:"]', '[ng-href*="playerId:"]', '[data-route*="playerId:"]', '[clickable*="playerId"]', '[ng-click*="playerId"]', '.playerName', '.player-name', '.player', '.ownerName', '.attacker', '.defender', 'td.player'].join(',');
  let observer = null;
  let scanTimer = null;
  let indexTimer = null;
  ...
  function enabled() {
    return typeof window.isQolEnabled === 'function' && window.isQolEnabled('watchlist');
  }
  ...
  function rebuildIndex(force = false) {
    ...
    try {
      ...
      console.warn('[APES Watchlist Highlights] Could not read Watchlist data.', error);
    }
        ...
        if (Array.isArray(tabs)) {
          tabs.forEach(tab => {
            ...
            tab.entries.forEach(entry => {
              ...
              const playerName = normalizeName(entry?.playerName);
              if (playerId) {
                nextById.set(playerId, mergeEntry(nextById.get(playerId), entry, tab));
              }
              if (playerName) {
                nextByName.set(playerName, mergeEntry(nextByName.get(playerName), entry, tab));
              }
            });
          });
        }
      } catch (error) {
        console.warn('[APES Watchlist Highlights] Invalid Watchlist data.', error);
      }
    }
    byPlayerId = nextById;
  ...
  function extractPlayerId(element) {
    const attributes = ['data-player-id', 'data-playerid', 'href', 'ng-href', 'data-route', 'clickable', 'ng-click'];
    for (const attribute of attributes) {
      const value = element.getAttribute?.(attribute);
      if (!value) {
        continue;
      }
      const explicitMatch = value.match(/playerId(?:\s*[:=]\s*|\D{1,8})(\d+)/i);
      if (explicitMatch) {
        return explicitMatch[1];
      }
      const profileMatch = value.match(/(?:openPlayer|openProfile|playerProfile)\D{0,12}(\d+)/i);
      if (profileMatch) {
  ...
  function isApesInterface(element) {
    return Boolean(element.closest('#qol-watchlist-container, #qol-modal, ' + '#apes-v2-command-overlay, .qol-modal-overlay, ' + '.qol-wl-dropdown-menu'));
  }
  function restoreTitle(element) {
    ...
    const originalTitle = element.getAttribute('data-apes-watch-original-title');
    if (originalTitle) {
  ...
  function applyHighlight(element, info) {
    if (!element.hasAttribute('data-apes-watch-original-title')) {
      element.setAttribute('data-apes-watch-original-title', element.getAttribute('title') || '');
    }
    const tabList = info.tabs.join(', ');
    element.classList.add(HIGHLIGHT_CLASS);
    element.setAttribute('data-apes-watch-tabs', tabList);
    element.setAttribute('title', 'Watched player · ' + tabList);
  }
  function inspectCandidate(element) {
    if (!(element instanceof Element) || isApesInterface(element)) {
      return;
    }
  ...
  function isExcludedTextNode(node) {
    ...
    return !parent || isApesInterface(parent) || Boolean(parent.closest('script, style, noscript, input, textarea, select, option, ' + '[contenteditable="true"], [aria-hidden="true"]'));
  }
  ...
  function refreshTextHighlights() {
    clearTextHighlights();
    const watchedNames = Array.from(byPlayerName.keys()).filter(Boolean).sort((a, b) => b.length - a.length);
    if (!watchedNames.length || !document.body) {
      return;
    }
    const supportsTextHighlights = typeof window.Highlight === 'function' && Boolean(window.CSS?.highlights);
    const ranges = [];
    ...
    while (node) {
      if (!isExcludedTextNode(node)) {
        ...
        if (exactInfo && nodeName === parentName && !parent.classList.contains(HIGHLIGHT_CLASS)) {
          applyHighlight(parent, exactInfo);
        } else if (supportsTextHighlights && !parent.closest('.' + HIGHLIGHT_CLASS)) {
          ...
          watchedNames.forEach(name => {
            ...
            while (index !== -1) {
              ...
              if (!overlaps && isNameBoundary(lowerText[index - 1]) && isNameBoundary(lowerText[end])) {
    ...
    if (supportsTextHighlights && ranges.length) {
      window.CSS.highlights.set(TEXT_HIGHLIGHT_NAME, new window.Highlight(...ranges));
    }
  }
  function scanRoot(root = document) {
    ...
    root.querySelectorAll?.(CANDIDATE_SELECTOR).forEach(inspectCandidate);
  ...
  function clearHighlights() {
    clearTextHighlights();
    document.querySelectorAll('.' + HIGHLIGHT_CLASS).forEach(removeHighlight);
  }
