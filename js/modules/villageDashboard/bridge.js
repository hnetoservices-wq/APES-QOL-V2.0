(() => {
  ...
  function safeCopy(value, depth = 0, seen = new WeakSet()) {
    ...
    if (Array.isArray(value)) {
      return value.slice(0, 220).map(item => safeCopy(item, depth + 1, seen)).filter(item => item !== undefined);
    }
  ...
  function compactBuildingQueue(queue) {
    ...
    for (const bucket of Object.values(queues)) {
      ...
      for (const item of bucket) {
        ...
        if (timeStart !== null && timeStart > 0 && finished !== null && finished > nowSeconds) {
  ...
  function extractIdsFromOwnCollection(cacheObject) {
    ...
    function walk(value, depth = 0) {
      ...
      if (typeof value === 'number' || typeof value === 'string') {
        ...
        if (named) ids.add(named);else if (/^\d{6,}$/.test(text)) ids.add(text);
        return;
      }
      if (typeof value !== 'object') return;
      if (seen.has(value)) return;
      seen.add(value);
      const direct = asId(value.villageId ?? value.locationId ?? value.id ?? value?.data?.villageId ?? value?.data?.locationId);
  ...
  function activeVillageId(cacheObject) {
    const fromHash = String(window.location.hash || '').match(/(?:^|\/)villId:(\d+)/i)?.[1];
  ...
  function ownVillageIds(cacheObject) {
    ...
    const validCollectionIds = [...fromCollection].filter(id => modelData(cacheObject, `Village:${id}`));
    if (validCollectionIds.length) {
      return {
        playerId: ownPlayerId ?? asFiniteNumber(modelData(cacheObject, `Village:${validCollectionIds[0]}`)?.playerId),
  ...
  function resolveCollectionModels(cacheObject, key, modelPrefix) {
    ...
    for (const value of values) {
      ...
      if (typeof value === 'number' || typeof value === 'string') {
        ...
        id = asId(value.troopId ?? value.buildingId ?? value.id ?? value?.data?.troopId ?? value?.data?.buildingId);
        if (!id) {
          id = String(value.name ?? value.key ?? '').match(new RegExp(`${modelPrefix}:(\\d+)`, 'i'))?.[1] || '';
        }
      }
      if (!id) continue;
  ...
  function villageSnapshot(cacheObject, villageId, playerId) {
    ...
    const buildings = resolveCollectionModels(cacheObject, `Collection:Building:${villageId}`, 'Building').map(compactBuilding);
    const stationaryTroops = resolveCollectionModels(cacheObject, `Collection:Troops:stationary:${villageId}`, 'Troops').map(compactTroop);
    const movingTroops = resolveCollectionModels(cacheObject, `Collection:Troops:moving:${villageId}`, 'Troops').map(compactTroop);
    const elsewhereTroops = resolveCollectionModels(cacheObject, `Collection:Troops:elsewhere:${villageId}`, 'Troops').map(compactTroop);
    const woundedTroops = resolveCollectionModels(cacheObject, `Collection:Troops:wounded:${villageId}`, 'Troops').map(compactTroop);
    return {
      villageId,
  ...
  function buildSnapshot() {
    ...
    const activeId = activeVillageId(cacheObject);
    const villages = ownership.ids.map(id => villageSnapshot(cacheObject, id, ownership.playerId)).sort((a, b) => {
