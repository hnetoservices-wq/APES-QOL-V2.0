(() => {
  ...
  function isAlarmFeatureEnabled() {
    ...
    try {
      return localStorage.getItem('qol_buildingAlarm') !== 'false';
    } catch (_) {
      return true;
    }
  }
  function readAlarms() {
    ...
    try {
      ...
      return parsed.filter(alarm => alarm && Number.isFinite(Number(alarm.alarmAt)) && Number.isFinite(Number(alarm.finishAt))).filter(alarm => Number(alarm.finishAt) > now).sort((a, b) => Number(a.alarmAt) - Number(b.alarmAt));
    } catch (_) {
      return [];
    }
  }
  ...
  function queueItems(queue) {
    ...
    const walkBucket = bucket => {
      ...
      if ('locationId' in bucket || 'finished' in bucket || 'finishTime' in bucket) {
        add(bucket);
        return;
      }
    ...
    if (Array.isArray(source)) source.forEach(walkBucket);else Object.values(source).forEach(walkBucket);
    return items;
  }
  ...
  function eventNode(event) {
    ...
    node.title = Number.isFinite(event.location) ? `Open ${event.transition} in ${event.villageName}` : `Switch to ${event.villageName}`;
  ...
  function openAlarmTarget(node) {
    ...
    location.hash = /^\d+$/.test(locationId) ? `#/page:village/villId:${villageId}/location:${locationId}/window:building` : `#/page:village/villId:${villageId}`;
  }
  ...
  function syncTimeline() {
    ...
    all.forEach((node, index) => {
      if (index < MAX_EVENTS) list.appendChild(node);else node.remove();
    });
