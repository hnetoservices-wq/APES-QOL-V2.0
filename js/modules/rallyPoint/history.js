(() => {
  ...
  async function loadSnapshots() {
    ...
    loadingPromise = (async () => {
      ...
      snapshots = Array.isArray(stored) ? stored.filter(item => item && KINDS[item.kind] && Array.isArray(item.items)) : [];
      loaded = true;
      loadingPromise = null;
      return snapshots;
    })();
  ...
  function getVillageContext() {
    ...
    try {
      ...
      const current = Array.isArray(villages) ? villages.find(village => String(village?.villageId || '') === villageId) : null;
      if (current) {
  ...
  function baseIdentity(kind, item) {
    return [kind, normalize(item.subject), normalize(item.village), normalize(item.type), normalize(item.landing)].join('|');
  }
  ...
  function snapshotList(kind) {
    return snapshots.filter(snapshot => snapshot.kind === kind).sort((a, b) => Number(b.createdAt || 0) - Number(a.createdAt || 0));
  }
  function compareSnapshots(a, b) {
    ...
    rows.sort((left, right) => {
      return order[left.status] - order[right.status] || String(left.item.landing || '').localeCompare(String(right.item.landing || '')) || String(left.item.subject || '').localeCompare(String(right.item.subject || ''));
    });
    return {
      rows,
      newItems,
...
`;
  ...
  function comparisonRowHtml(row, kind) {
    const config = KINDS[kind];
    const remaining = row.status === 'unchanged' ? `${escapeHtml(row.previous?.remaining || '-')} → ${escapeHtml(row.item.remaining || '-')}` : escapeHtml(row.item.remaining || '-');
  ...
  function renderHistory() {
    ...
    const {
      ...
      a,
      b
    } = getSelectedSnapshots(selectedKind);
    const canCompare = Boolean(a && b);
    const comparison = canCompare ? compareSnapshots(a, b) : {
      rows: [],
      newItems: [],
      removedItems: [],
      unchangedItems: []
