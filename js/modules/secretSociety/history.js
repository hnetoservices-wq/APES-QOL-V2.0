(() => {
  ...
  function historyForSociety(societyId) {
    ...
    return Array.isArray(society?.snapshots) ? society.snapshots.slice().sort((a, b) => Number(a.scannedAt) - Number(b.scannedAt)) : [];
  }
  ...
  function archiveCurrentScans() {
    ...
    const signature = scans.map(scan => `${cleanText(scan?.id)}:${Number(scan?.scannedAt) || 0}:${Array.isArray(scan?.members) ? scan.members.length : 0}`).sort().join('|');
    ...
    scans.forEach(scan => {
      ...
      const members = Array.isArray(scan?.members) ? scan.members.map(compactMember).filter(member => member.name) : [];
      if (!id || !Number.isFinite(scannedAt) || scannedAt <= 0 || !members.length) return;
      const society = root[server][id] && typeof root[server][id] === 'object' ? root[server][id] : {
        id,
        name: cleanText(scan?.name) || 'Secret Society',
        snapshots: []
      };
      society.id = id;
      ...
      if (!society.snapshots.some(snapshot => Number(snapshot?.scannedAt) === scannedAt)) {
        ...
        society.snapshots = society.snapshots.filter(snapshot => Number.isFinite(Number(snapshot?.scannedAt))).sort((a, b) => Number(a.scannedAt) - Number(b.scannedAt)).slice(-HISTORY_LIMIT);
        changed = true;
      }
  ...
  function memberChanges(snapshotA, snapshotB) {
    ...
    const sort = (x, y) => x.name.localeCompare(y.name, undefined, {
      numeric: true,
      sensitivity: 'base'
    });
    joined.sort(sort);
  ...
  function populateLaterSelect(dialog, societyId) {
    ...
    const aTime = Number(selectA.value);
    const later = selectA.value ? snapshots.filter(snapshot => Number(snapshot.scannedAt) > aTime) : [];
    selectB.innerHTML = ['<option value="">Select Scan B…</option>', ...later.map(snapshot => `<option value="${Number(snapshot.scannedAt)}">${escapeHtml(formatScanDate(snapshot.scannedAt))}</option>`)].join('');
    selectB.value = '';
  ...
  function openCompareDialog(societyId) {
    ...
    const selectB = dialog.querySelector('[data-compare-b]');
    const run = dialog.querySelector('[data-compare-run]');
    selectA.innerHTML = ['<option value="">Select Scan A…</option>', ...snapshots.slice(0, -1).map(snapshot => `<option value="${Number(snapshot.scannedAt)}">${escapeHtml(formatScanDate(snapshot.scannedAt))}</option>`)].join('');
    selectA.value = '';
    ...
    selectB.addEventListener('change', () => {
      const valid = Boolean(selectA.value && selectB.value && Number(selectB.value) > Number(selectA.value));
      run.setAttribute('aria-disabled', String(!valid));
      resetComparisonResult(dialog);
    });
    const compare = event => {
  ...
  function injectPanelHistoryUi() {
    ...
    compare.title = snapshots.length < 2 ? 'At least two stored scans are required.' : `${snapshots.length} stored scans available`;
  ...
  function sync() {
    ...
    if (changed) scheduleUiRefresh();else injectPanelHistoryUi();
  }
  function start() {
    injectStyles();
    sync();
    ...
    window.addEventListener('qol_setting_changed', event => {
      ...
      if (event.detail.enabled) sync();else document.getElementById(DIALOG_ID)?.remove();
    });
  }
