(() => {
  ...
  const ROADMAP_STORAGE_OPTIONS = Object.freeze({
    feature: FEATURE_KEY,
    key: 'roadmaps',
    scope: 'player'
  });
  ...
  const RING_GAP = 120;
  const RESOURCE_BUILDING_TYPES = Object.freeze({
    wood: 1,
    clay: 2,
    iron: 3,
    crop: 4
  });
  const PRODUCTION_BUILDING_TYPES = Object.freeze({
    sawmill: 5,
    brickyard: 6,
    foundry: 7,
    mill: 8,
  ...
  function setEnabled(value) {
    try {
      localStorage.setItem(`qol_${FEATURE_KEY}`, String(Boolean(value)));
    } catch (_) {}
  ...
  function injectCollapsedToolbarEntry() {
    ...
    if (settings) dropdown.insertBefore(item, settings);else dropdown.appendChild(item);
  }
  function visibleToolbarControls() {
    const selectors = ['#qol-help-toggle-btn', '#qol-rally-point-toggle-btn', '#qol-watchlist-toggle', '#qol-checklist-toggle-btn', '#qol-npc-calc-toggle-btn', '#qol-distance-calc-toggle-btn', '#qol-oasis-toggle-btn', '#qol-report-archive-toggle', '#qol-cp-toggle-btn', '#qol-ss-scanner-toggle-btn', '#qol-tribe-skins-toggle-btn'].join(',');
    return [...document.querySelectorAll(selectors)].map(element => ({
      element,
      rect: element.getBoundingClientRect()
    })).filter(item => item.rect.width > 0 && item.rect.height > 0).filter(item => {
  ...
  async function loadRoadmaps() {
    ...
    roadmapLoadPromise = (async () => {
      ...
      if (!saved) {
        try {
          saved = JSON.parse(localStorage.getItem(ROADMAP_FALLBACK_KEY) || 'null');
        } catch (_) {}
      }
  ...
  async function saveRoadmaps() {
    ...
    try {
      localStorage.setItem(ROADMAP_FALLBACK_KEY, JSON.stringify(snapshot));
    } catch (_) {}
  }
  function currentVillageIdentity() {
    ...
    const domName = String(document.querySelector('.currentVillageName .dropdownHead .selectedItem .villageEntry, #villageList .dropdownHead .selectedItem .villageEntry')?.textContent || '').trim();
    return {
      villageId,
  ...
  function roadmapStepId(row) {
    return [row.kind, row.resource || row.building || '', Number.isInteger(Number(row.index)) ? Number(row.index) : '', Number(row.fromLevel) || 0, Number(row.toLevel) || 0].join(':');
  }
  ...
  function evaluateRoadmapStep(village, roadmap, step) {
    ...
    matching.forEach((item, index) => {
      ...
      if (target >= step.toLevel && !queuedMatch) queuedMatch = {
        item,
        target
      };
    });
  ...
  function roadmapCheckbox(step, evaluation) {
    ...
    const title = evaluation.auto && evaluation.status === 'complete' ? 'Automatically completed from the current village level' : 'Mark this roadmap step complete manually';
  ...
  function roadmapViewHtml(roadmap, village) {
    ...
    const evaluations = roadmap.steps.map(step => ({
      step,
      evaluation: evaluateRoadmapStep(village, roadmap, step)
    }));
    ...
    const rows = evaluations.map(({
      ...
      const queuedTime = evaluation.status === 'queued' && evaluation.finishTime ? `<span class="qol-rup-roadmap-queue-time">${formatCountdown(evaluation.finishTime)}</span>` : '';
    ...
    const liveText = village ? 'Live cache connected' : 'Waiting for village cache';
    return `
            ...
            ${counts.complete === roadmap.steps.length && roadmap.steps.length ? '<div class="qol-rup-roadmap-complete-banner">Roadmap complete. This village has reached every saved recommendation.</div>' : ''}
  ...
  async function setManualRoadmapCompletion(stepId, completed) {
    ...
    if (completed) done.add(String(stepId));else done.delete(String(stepId));
