/**
 * APES QoL v2 — Account Operations Center
 *
 * Press H to toggle a borderless account-wide village operations view.
 * Live village/cache data comes from villageDashboardBridge.js. Scan Now
 * deliberately visits every village once to refresh Travian's cache and save
 * device-local production/storage plus quick-access building locations.
 */
(() => {
    'use strict';
    ...
    const BUILDING_NAMES = Object.freeze({
        1:'Woodcutter', 2:'Clay Pit', 3:'Iron Mine', 4:'Cropland', 5:'Sawmill',
        6:'Brickyard', 7:'Iron Foundry', 8:'Grain Mill', 9:'Bakery', 10:'Warehouse',
        11:'Granary', 12:'Smithy', 14:'Tournament Square', 15:'Main Building',
        16:'Rally Point', 17:'Marketplace', 18:'Embassy', 19:'Barracks', 20:'Stable',
        21:'Workshop', 22:'Academy', 23:'Cranny', 24:'Town Hall', 25:'Residence',
        26:'Palace', 27:'Treasury', 28:'Trade Office', 29:'Great Barracks',
        30:'Great Stable', 31:'City Wall', 32:'Earth Wall', 33:'Palisade',
        34:'Stonemason', 35:'Brewery', 36:'Trapper', 37:"Hero's Mansion",
        38:'Great Warehouse', 39:'Great Granary', 40:'Wonder of the World',
        41:'Horse Drinking Trough', 46:'Healing Tent'
    });

    const BUILDING_SHORT_NAMES = Object.freeze({ 1:'Wood', 2:'Clay', 3:'Iron', 4:'Crop' });
    ...
    function enabled() {
        try { return localStorage.getItem(`qol_${SETTING_KEY}`) !== 'false'; }
        catch (_) { return true; }
    }
    ...
    function sortStorageKey() { return `apes_aoc_sort:${location.hostname}`; }
    ...
    function saveSortMode(value) {
        ...
        try { localStorage.setItem(sortStorageKey(), sortMode); } catch (_) {}
    }
    ...
    function writeScanStore(store) {
        ...
        scanStoreCacheKey = key;
        try { localStorage.setItem(key, JSON.stringify(scanStoreCache)); }
        catch (error) { console.warn('[APES AOC] Could not save scan data.', error); }
    }
    ...
    function scanResourcesFromDom() {
        return RESOURCE_DEFS.map(resource => {
            ...
            return { key:resource.key, name:resource.name, current, capacity, production };
        }).filter(Boolean);
    }
    ...
    function constructionQueueItems(queue) {
        ...
        const pushBucket = bucket => {
            ...
            if (queueLocation(bucket) !== null) { items.push(bucket); return; }
    ...
    function constructionEntries(village) {
        ...
        for (const item of constructionQueueItems(village?.buildingQueue)) {
            ...
            let level = null;
            if (currentLevel !== null && levelDifference !== null) level = currentLevel + levelDifference + 1;
            else if (currentLevel !== null) level = currentLevel + sequenceIndex + 1;
            else {
        ...
        for (const building of village?.buildings || []) {
            ...
            entries.push({
                type, location:locationId,
                label:BUILDING_SHORT_NAMES[type] || fullLabel,
                fullLabel, level:current !== null ? current + 1 : null,
                end:null, waiting:false
            });
        }
        return entries;
    }
    ...
    function primitiveUnitCounts(root, tribeId) {
        ...
        function walk(node, depth = 0) {
            ...
            if (Array.isArray(node)) {
                ...
                if (primitives) { node.forEach((value,index) => addUnitCount(totals, tribeId, index + 1, value)); return; }
            ...
            for (const [key,value] of Object.entries(node)) {
                if (/^\d+$/.test(key) && ['number','string'].includes(typeof value)) addUnitCount(totals, tribeId, Number(key), value);
    ...
    function unitQueueEntries(village) {
        ...
        for (const item of candidates) {
            ...
            seen.add(key);
            entries.push({ rawUnit, localIndex:local, label:rawUnit !== null ? unitName(village?.tribeId,rawUnit) : 'Troops', amount, building:BUILDING_NAMES[buildingType] || '', end });
        }
        if (!entries.length) {
            primitiveUnitCounts(queue?.unitsInQueue, village?.tribeId).forEach((amount, local) => {
                entries.push({ rawUnit:local, localIndex:local, label:unitName(village?.tribeId,local), amount, building:'', end:null });
            });
        }
    ...
    function smithyInfo(village) {
        ...
        if (!candidates.length) return { label:'Upgrade active', end:null };
        ...
        return { label:`${label}${level !== null ? ` → ${level}` : ''}`, end:findEndTime(item) };
    }
    ...
    function dashboardBuildingEntries(village) {
        ...
        if (Array.isArray(scanned) && scanned.length) {
            return DASHBOARD_BUILDINGS.map(definition => {
                ...
                return building && Number.isFinite(Number(building.location)) ? { ...definition, location:Number(building.location), level:asNumber(building.level) } : null;
            }).filter(Boolean);
        }
        return DASHBOARD_BUILDINGS.map(definition => {
            const building = buildingByType(village, definition.type);
            const locationId = asNumber(building?.locationId);
            return locationId === null ? null : { ...definition, location:locationId, level:asNumber(building?.lvl) };
        }).filter(Boolean);
    }
    ...
    function projectedResourceStates(village) {
        ...
        return scan.resources.map(resource => {
            ...
            return { ...resource, current:projected, capacity, production, etaMs, state, scannedAt:Number(scan.scannedAt) };
        }).filter(Boolean);
    }
    ...
    function makeAlert(key, label, tone, weight, title = '') {
        return { key, label, tone, weight, title: title || label };
    }
    function villageInsights(village) {
        ...
        resources.forEach(resource => {
            ...
            if (resource.state === 'full') alerts.push(makeAlert(`full:${resource.key}`,`${resource.name} full`,'danger',90));
            else if (resource.state === 'empty') alerts.push(makeAlert(`empty:${resource.key}`,`${resource.name} empty`,'danger',90));
            else if (resource.etaMs <= STORAGE_RISK_MS) {
        ...
        if (townHall && !celebration.active) {
            if (celebration.end && now - celebration.end <= RECENT_CELEBRATION_FINISH_MS) alerts.push(makeAlert('celebrationFinished','Celebration finished','warn',35));
            else alerts.push(makeAlert('townHallIdle','Town Hall idle','info',20));
        }
        ...
        if (!Number.isFinite(age)) alerts.push(makeAlert('scanNeeded','Scan needed','info',15,'Run Scan Now to refresh production/storage data on this device.'));
        else if (age > SCAN_STALE_MS) alerts.push(makeAlert('scanStale',`Scan ${formatCompactAge(age)} old`,'info',15));
        ...
        activeConstructions.forEach(entry => {
            ...
            if (freeAt > now) events.push({ at:freeAt, villageId:String(village.villageId), villageName:village.name, label:`Free finish: ${display}`, type:'free' });
            else events.push({ at:now, villageId:String(village.villageId), villageName:village.name, label:`Free finish ready: ${display}`, type:'ready' });
            events.push({ at:end, villageId:String(village.villageId), villageName:village.name, label:`${display} finishes`, type:'construction' });
        });
        training.forEach(entry => {
            const end = asTimestamp(entry.end);
            if (end && end > now) events.push({ at:end, villageId:String(village.villageId), villageName:village.name, label:`Training finishes${entry.label ? `: ${entry.label}` : ''}`, type:'training' });
        });
        const smithyEnd = asTimestamp(smithy?.end);
        if (smithyEnd && smithyEnd > now) events.push({ at:smithyEnd, villageId:String(village.villageId), villageName:village.name, label:`Smithy finishes: ${smithy.label}`, type:'smithy' });
        if (celebration.active && celebration.end) events.push({ at:celebration.end, villageId:String(village.villageId), villageName:village.name, label:`${celebration.name} finishes`, type:'celebration' });
        ...
        return { alerts, events, attentionScore, nextEvent, constructionFinish, storageRiskMs, freeReady:freeReady.length, cropDeficit:Boolean(crop && crop.production < 0) };
    }
    ...
    function sortedVillageModels(villages) {
        const models = villages.map((village,index) => ({ village, index, insights:villageInsights(village) }));
    ...
    function renderSummary(models) {
        ...
        target.innerHTML = [
            ['Needs attention', attention, attention ? 'warn' : 'ready'],
            ['Free finishes', free, free ? 'ready' : 'muted'],
            ['Crop deficits', crop, crop ? 'danger' : 'muted'],
            ['Storage risks', storage, storage ? 'warn' : 'muted']
        ].map(([label,value,tone]) => `<span class="apes-aoc-summary-item apes-aoc-${tone}"><strong>${value}</strong>${label}</span>`).join('');
    }
    ...
    function renderEvents(models) {
        ...
        if (!target) return;
        const now = Date.now();
        const events = models.flatMap(model => model.insights.events)
            .filter(event => event.at >= now - 1000)
            .sort((a,b) => a.at - b.at)
            .slice(0,8);
        if (!events.length) {
    ...
    function mountDashboard() {
        ...
        sort.value = sortMode;
        sort.addEventListener('change', () => { saveSortMode(sort.value); renderDashboard(); });

        overlay.addEventListener('click', event => {
            if (event.target === overlay) { closeDashboard(); return; }
            const scanButton = event.target.closest('.apes-vd-scan-btn');
            if (scanButton) { event.preventDefault(); event.stopPropagation(); scanAllVillages(); return; }
            const eventCard = event.target.closest('.apes-aoc-event[data-event-village-id]');
            if (eventCard && !scanInProgress) { openVillage(eventCard.dataset.eventVillageId); return; }
        ...
        overlay.addEventListener('keydown', event => {
            const scanButton = event.target.closest?.('.apes-vd-scan-btn');
            if (scanButton && ['Enter',' '].includes(event.key)) { event.preventDefault(); scanAllVillages(); return; }
            const buildingControl = event.target.closest?.('.apes-vd-building-link');
            if (buildingControl && !scanInProgress && ['Enter',' '].includes(event.key)) {
                event.preventDefault(); openBuilding(buildingControl.dataset.buildingVillageId, buildingControl.dataset.buildingLocation); return;
            }
            const villageControl = event.target.closest?.('[data-village-id]');
            if (villageControl && !scanInProgress && ['Enter',' '].includes(event.key)) { event.preventDefault(); openVillage(villageControl.dataset.villageId); }
        });
        updateScanButton();
        return overlay;
    }
    ...
    function renderDashboard() {
        ...
        body.innerHTML = models.map(({ village, insights }) => {
            const villageId = String(village.villageId || '');
    ...
    async function scanAllVillages() {
        ...
        try {
            for (let index = 0; index < orderedIds.length; index += 1) {
                ...
                scanProgress = { current:index + 1, total:orderedIds.length, returning:false };
                updateScanButton();
            ...
            if (/^\d+$/.test(startVillageId) && currentVillageIdFromUrl() !== startVillageId) {
                scanProgress = { current:orderedIds.length, total:orderedIds.length, returning:true };
                updateScanButton();
    ...
    function startRefresh() {
        ...
        retryTimer = setTimeout(() => { if (!snapshot?.villages?.length && isOpen()) requestSnapshot(); },350);
    }
    ...
    function isOpen() { return document.getElementById(OVERLAY_ID)?.classList.contains('open') || false; }
    ...
    function toggleDashboard() { isOpen() ? closeDashboard() : openDashboard(); }
    ...
    window.addEventListener('keydown', event => {
        ...
        event.preventDefault(); event.stopImmediatePropagation(); closeDashboard();
    }, true);
    window.addEventListener('hashchange', () => { if (isOpen()) requestSnapshot(); });
    window.addEventListener('qol_close_others', event => { if (event.detail?.source !== 'villagePalette') closeDashboard(); });
    window.addEventListener('qol_setting_changed', event => { if (event.detail?.key === SETTING_KEY && !event.detail.enabled) closeDashboard(); });
