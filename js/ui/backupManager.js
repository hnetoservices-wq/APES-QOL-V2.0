(() => {
  ...
  function collectExtensionStorage() {
    return new Promise(resolve => {
      ...
      chrome.storage.local.get(null, stored => {
        ...
        resolve(Object.fromEntries(Object.entries(stored || {}).filter(([key]) => isApesKey(key))));
      });
    });
  }
  ...
  function backupFilename(payload) {
    ...
    const server = String(payload?.source?.hostname || 'server').replace(/[^a-z0-9.-]+/gi, '_');
  ...
  async function exportBackup() {
    ...
    try {
      ...
      setStatus(`Exported ${summary.totalCount} persistent APES entries (${formatBytes(summary.bytes)}).`, 'success');
      updateStats(payload);
    } catch (error) {
  ...
  function sanitizeBackupObject(value) {
    ...
    return Object.fromEntries(Object.entries(value).filter(([key]) => isApesKey(key)));
  }
  ...
  async function chooseImportFile(file) {
    ...
    try {
      ...
      const date = Number(payload.exportedAt) ? new Date(Number(payload.exportedAt)).toLocaleString() : 'unknown date';
      const serverNote = sameServer ? `${summary.localCount} current-server entries + ${summary.extensionCount} extension entries will be merged.` : `${summary.extensionCount} extension entries will be merged. ${skippedLocal} local entries are for ${sourceHost} and will be skipped here; import this file while visiting that server to restore them.`;
      setStatus(`Backup ready · ${date} · ${serverNote} Click Confirm Import to continue.`, 'warning');
    } catch (error) {
