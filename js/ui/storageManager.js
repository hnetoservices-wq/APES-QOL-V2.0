(() => {
  ...
  const CATEGORIES = [['watchlists', 'Watchlists', 'Saved players, tabs, villages and notes.', key => key.startsWith('qol_watchlist_')], ['reports', 'Report Archive', 'Archived report folders and report copies.', key => key.startsWith('qol_report_archive_')], ['checklists', 'Checklists', 'Custom checklists and completed-step progress.', key => key === 'qol_custom_checklists' || key === 'qol_checklist_progress'], ['alarms', 'Building Alarms', 'Active and instant-finish-ready alarms.', key => key === 'qol_building_alarms'], ['igm', 'IGM Organization', 'Conversation folders and message tags.', key => key === 'qol_conversation_tags' || key === 'qol_custom_chat_tags'], ['oasis', 'Oasis Scanner', 'Scanned tiles, croppers, oases and tag-team data.', key => key.startsWith('qol_oasis_') || key.startsWith('qol_cropper_') || key.startsWith('qol_tile_')], ['society', 'Secret Society Scanner', 'Saved Secret Society member scans.', key => key.startsWith('apes_secret_society_scans')], ['visual', 'Visual Asset Cache', 'Cached tribe-skin asset catalogue.', key => key.startsWith('apes_visual_tribe_skin_assets')], ['preferences', 'Preferences', 'Feature toggles, keybinds and interface choices.', key => key.startsWith('qol_') || key.includes('skin_selection') || key.includes(':global:'), true], ['other', 'Other APES Data', 'Other APES-owned data and legacy caches.', () => true]].map(([id, name, description, match, protectedData = false]) => ({
    id,
    name,
    description,
    match,
  ...
  function readExtensionStorage() {
    return new Promise(resolve => {
      ...
      chrome.storage.local.get(null, stored => {
        ...
        resolve(Object.entries(stored || {}).filter(([key]) => isCurrentServerExtensionKey(key)).map(([key, value]) => ({
          source: 'extension',
          key,
          value,
          bytes: sizeOf(key, value),
  ...
  async function collectInventory() {
    const entries = [];
    try {
      entries.push(...readWebStorage(localStorage, 'local'));
    } catch (error) {
      console.warn('[APES Storage Manager] localStorage unavailable.', error);
    }
    try {
      entries.push(...readWebStorage(sessionStorage, 'session'));
    } catch (error) {
  ...
  function updateSelection() {
    ...
    overlay.querySelectorAll('[data-storage-category]').forEach(row => {
      ...
      row.querySelector('.apes-storage-checkbox')?.setAttribute('aria-checked', String(checked));
    });
    ...
    status.textContent = entries.length ? `${entries.length} entries · ${formatBytes(bytes)} selected` : 'Nothing selected';
  ...
  function renderInventory() {
    ...
    overlay.querySelector('.apes-storage-total').innerHTML = `<strong>${formatBytes(totalBytes)}</strong> across ${inventory.length} saved ${inventory.length === 1 ? 'entry' : 'entries'}`;
    const summaries = CATEGORIES.map(categorySummary).filter(summary => summary.entries.length || summary.id === 'preferences');
  ...
  async function clearSelected() {
    ...
    entries.forEach(entry => {
      if (entry.source === 'local') localStorage.removeItem(entry.key);else if (entry.source === 'session') sessionStorage.removeItem(entry.key);else extensionKeys.push(entry.key);
    });
    ...
    document.querySelector(`#${OVERLAY_ID} .apes-storage-status`).textContent = `${entries.length} entries removed. Reloading...`;
  ...
  function labelLauncher() {
    ...
    if (button.title !== 'Review and selectively remove APES saved data') {
