(() => {
  ...
  async function waitForPlayerId(timeoutMs = PLAYER_ID_TIMEOUT_MS) {
    ...
    throw new Error('APES could not resolve the logged-in player identity. ' + 'Player-scoped data was not read or written to avoid account mixing.');
  }
  ...
  async function auditStorageScopes() {
    ...
    const currentServerKeys = currentServerPrefix ? keys.filter(key => key.startsWith(currentServerPrefix)) : [];
    const ambiguousPlayerKeys = currentServerKeys.filter(key => key.startsWith(`${PREFIX}:${server}:unknown:`));
    const currentPlayerKeys = /^\d+$/.test(playerId) ? currentServerKeys.filter(key => key.startsWith(`${PREFIX}:${server}:${playerId}:`)) : [];
    const serverScopedKeys = currentServerKeys.filter(key => key.startsWith(`${PREFIX}:${server}:server:`));
  ...
  APES.storage = {
    ...
    async get(options, fallback = null) {
      ...
      return Object.hasOwn(result, storageKey) ? result[storageKey] : fallback;
    },
    ...
    async remove(options) {
      ...
      APES.events.emit('storage:removed', {
        key: storageKey
      });
    },
    async list({
      ...
      const all = await getAllChromeStorage();
      return Object.fromEntries(Object.entries(all).filter(([key]) => {
  ...
  window.setTimeout(() => {
    auditStorageScopes().then(report => {
      ...
      console.warn('[APES.storage] Ambiguous legacy v2 player keys detected. ' + 'They were left untouched and should be reviewed manually:', report.ambiguousPlayerKeys);
    }).catch(() => {});
  }, 1500);
})();
