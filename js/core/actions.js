  ...
  APES.actions = {
    register(action) {
      if (!action?.id || !action?.label || typeof action.run !== 'function') {
        throw new Error('APES actions require id, label, and run properties.');
      }
      const normalized = Object.freeze({
        ...
        enabled: typeof action.enabled === 'function' ? action.enabled : () => true,
        run: action.run
      });
      registry.set(normalized.id, normalized);
      APES.events.emit('actions:changed', {
        id: normalized.id
      });
    ...
    list() {
      return Array.from(registry.values()).filter(action => action.enabled());
    },
    search(query = '') {
      const wanted = normalizeText(query);
      return this.list().filter(action => {
        const haystack = [action.label, action.description, action.group, ...action.keywords].map(normalizeText).join(' ');
    ...
    async run(actionId, context) {
      ...
      APES.events.emit('action:started', {
        id: action.id
      });
      try {
        const result = await action.run(context);
        APES.events.emit('action:completed', {
          id: action.id
        });
        return result;
      } catch (error) {
