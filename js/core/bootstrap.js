(() => {
  ...
  const version = String(manifest.version_name || manifest.version || '2.0.0').trim();
  ...
  APES.release = Object.freeze({
    ...
    channel: /alpha/i.test(version) ? 'alpha' : /beta/i.test(version) ? 'beta' : 'stable'
  });
  APES.coreReady = true;
    ...
    emit(eventName, detail) {
      listeners.get(eventName)?.forEach(listener => {
        try {
          ...
          console.error(`[APES.events] Listener failed for ${eventName}:`, error);
        }
      });
      window.dispatchEvent(new CustomEvent(`apes:${eventName}`, {
        detail
      }));
    }
  };
  ...
  if (document.documentElement) beginVersionSync();else document.addEventListener('DOMContentLoaded', beginVersionSync, {
    once: true
  });
